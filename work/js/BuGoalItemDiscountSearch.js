/*** 
 *	檔案: buGoalItemDiscountSearch.js
 *	說明: 抽成率搜尋維護
 */
...
function headerInitial(){
	...
	vat.block.create( vnB_Header, {
		...
		title:"商品折數查詢作業", rows:[  
	 		{row_style:"", cols:[
	 			{items:[{name:"#L.itemdiscount",	type:"LABEL", 	value:"商品折數"}]},
				{items:[{name:"#F.itemdiscount",	type:"NUMM", bind:"itemdiscount", size:20}]},
			 	{items:[{name:"#L.discount", 	type:"LABEL", 	value:"降抽"}]},
			 	{items:[{name:"#F.discount", 	type:"NUMM", bind:"discount", size:20}]}
			]}
...
function detailInitial(){
  ...
  var vbCanGridModify = true;
	...
	vat.item.make(vnB_Detail, "itemdiscount"	, {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"商品折數"      });
	vat.item.make(vnB_Detail, "discount"  	    , {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"降抽"      });
	...
	vat.item.make(vnB_Detail, "discountId"      	, {type:"ROWID"});
	
	vat.block.pageLayout(vnB_Detail, {
														id                  : "vatDetailDiv",
														pageSize            : 10,
														searchKey           : ["discountId"],
														selectionType       : "CHECK",
														indexType           : "AUTO",
...
// 清除
function resetForm(){
	vat.item.setValueByName("#F.itemdiscount", "");
...
function loadBeforeAjxService(){
   // alert("loadBeforeAjxService");	
	...
	var processString = "process_object_name=buGoalItemDiscountService&process_object_method_name=getAJAXSearchPageData" + 
	                  "&itemdiscount"       + "=" + vat.item.getValueByName("#F.itemdiscount" ) +
...
function saveBeforeAjxService() {
	...
	var processString = "";
	if (checkEnableLine()) {
		processString = "process_object_name=buGoalItemDiscountService&process_object_method_name=saveSearchResult";
	}
	return processString;
}								
...
//alert("xxxxxxxxxxxxxxxxxxxxxxxxx");
	   vat.block.pageSearch(vnB_Detail, {
    		funcSuccess : 
    		    function(){
    		      vat.block.submit(
    		                     function(){ return "process_object_name=buGoalItemDiscountService&process_object_method_name=getSearchSelection";
