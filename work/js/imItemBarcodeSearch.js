...
function searchInitial(){ 
  ...
  if(typeof document.forms[0]["#loginBrandCode"] != 'undefined'){
	...
	vat.bean.init(	
  		function(){
			return "process_object_name=imItemBarcodeHeadAction&process_object_method_name=performSearchInitial"; 
    	},{
    		other: true
   	});   
  }
	...
	title:"補條碼查詢作業", rows:[  
	 		{row_style:"", cols:[
				...
				{items:[{name:"#F.orderNo", 		type:"TEXT"   , size:20}]},
			 	{items:[{name:"#L.dueDate", 	type:"LABEL"  , value:"到期日"}]},
				{items:[{name:"#F.dueDateStart", 	type:"DATE"   , size:10},
						{name:"#L.between", 		type:"LABEL"  , value:" 至 "},
						{name:"#F.dueDateEnd", 	type:"DATE"   , size:10}]},
...
function detailInitial(){
	...
	vat.item.make(vnB_Detail, "dueDate"    		, {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"到期日"      });
...
function loadBeforeAjxService(){
	...
	var processString = "process_object_name=imItemBarcodeHeadService&process_object_method_name=getAJAXSearchPageData" + 
	                  ...
	                  "&dueDateStart"      + "=" + vat.item.getValueByName("#F.dueDateStart"       ) +
	                  "&dueDateEnd"      	+ "=" + vat.item.getValueByName("#F.dueDateEnd"       ) +
...
function saveBeforeAjxService() {
	var processString = "";
	if (checkEnableLine()) {
		processString = "process_object_name=imItemBarcodeHeadService&process_object_method_name=saveSearchResult";
	}
	return processString;
}								
...
function doClosePicker(){
	   vat.block.pageSearch(vnB_Detail, {
    		funcSuccess : 
    		    function(){
    		      vat.block.submit(
    		                     function(){ return "process_object_name=imItemBarcodeHeadService&process_object_method_name=getSearchSelection";
...
function resetForm(){
    ...
    vat.item.setValueByName("#F.dueDateStart", "");
    vat.item.setValueByName("#F.dueDateEnd", "");
...
function openModifyPage(){
    ...
    var url = "/erp/Im_ItemBarcode:create:20100607.page?formId=" + vFormId + "&orderTypeCode="+orderTypeCode; 

     sc=window.open(url, '補條碼維護作業', 'menubar=no,resizable=no,scrollbars=no,status=no,resizable=yes,scrollbars=yes');
