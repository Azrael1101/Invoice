...
function kweDetail3(){
   ...
   // vbSelectionType = "CHECK";
    ...
    vat.item.make(vnB_Detail3, "indexNo"                   , {type:"IDX"  , size: '2%',                     desc:"序號"       });
    vat.item.make(vnB_Detail3, "checkbox"        , {type:"XBOX" , size: '1%' , desc:"啟用" , style:"background-color:#FFDF00;"});
    vat.item.make(vnB_Detail3, "menuId"            , {type:"TEXT" , size: '12%',  maxLen:20, desc:"選單編號", mode:"READONLY" });
	vat.item.make(vnB_Detail3, "menuName"            , {type:"TEXT" , size: '60%',  desc:"名稱", mode:"READONLY" });
	vat.item.make(vnB_Detail3, "url"            , {type:"TEXT" , size: '55%',  desc:"URL", mode:"READONLY" });
	vat.item.make(vnB_Detail3, "updateDate"            , {type:"DATE" , size: '25%',  desc:"更新日期", mode:"READONLY" });
	vat.item.make(vnB_Detail3, "brandCode"            , {type:"TEXT" , size: '2%',  desc:"品牌", mode:"READONLY" });
	vat.item.make(vnB_Detail3, "clickNumber"            , {type:"NUMM",dec:0	 , size: '2%',  desc:"次數", mode:"READONLY" });
	vat.item.make(vnB_Detail3, "cost"            , {type:"TEXT" , size: '2%',  desc:"成本", mode:"READONLY" });
	vat.item.make(vnB_Detail3, "categoryCode"            , {type:"TEXT" , size: '2%',  desc:"業種", mode:"READONLY" });
	vat.item.make(vnB_Detail3, "warehouseCode"            , {type:"TEXT" , size: '2%',  desc:"庫別", mode:"READONLY" });
	vat.item.make(vnB_Detail3, "lineId"                   , {type:"hidden"  ,                     desc:"lineId"       });
...
function selectAllMenus() {
    ...
    if (confirm("是否確定執行『全選』？")) {
        if (vat.item.getValueByName("#F.status") == "SAVE") {
            ...
            vat.block.submit(function () {
               return "process_object_name=buPurchaseServiceSub" + "&process_object_method_name=updateAllMenus";
            }, {
                other: true,
                picker: false,
                funcSuccess: function () {
...
// 全選
function selectAll() {
	//alert(document.getElementById('pickAll#8').checked);
	if(document.getElementById('pickAll#8').checked==true){
      document.getElementById('vatF#B8A#Y1#X2').checked=true;
      document.getElementById('vatF#B8A#Y2#X2').checked=true;
      document.getElementById('vatF#B8A#Y3#X2').checked=true;
      document.getElementById('vatF#B8A#Y4#X2').checked=true;
      document.getElementById('vatF#B8A#Y5#X2').checked=true;
      document.getElementById('vatF#B8A#Y6#X2').checked=true;
      document.getElementById('vatF#B8A#Y7#X2').checked=true;
      document.getElementById('vatF#B8A#Y8#X2').checked=true;
      document.getElementById('vatF#B8A#Y9#X2').checked=true;
      document.getElementById('vatF#B8A#Y10#X2').checked=true;
    }else{
     for(i=1;i<=10;i++){
       if(document.getElementById("vatF#B8A#Y"+i+"#X5").value.length==0){
          continue;
       }else{
          document.getElementById('vatF#B8A#Y'+i+'#X2').checked=false;
	   }
     }
...
function reconfirmImmovement(orderTypeCode,orderNo){
	...
	function reconfirmImmovement2(orderTypeCode,orderNo){
		if(confirm('是否要列印需求單')){	
			url2 = "http://10.1.94.161:8080/crystal/t2/IRQ.rpt?prompt0="+vat.item.getValueByName("#F.orderTypeCode")+"&prompt1="+vat.item.getValueByName("#F.orderNo");
...
function getOriginalData(roleCode){
  ...
  if(vat.item.getValueByName("#F.applicant")!=""){
    ...
    vat.ajax.XHRequest(
    {
        ...
        post:"process_object_name=buPurchaseServiceSub"+
                    "&process_object_method_name=executeCopyOrigMenu"+
                    "&brandCode=" + vat.item.getValueByName("#F.brandCode") + 
