...
function kweImSearchInitial(){
  if(typeof document.forms[0]["#loginBrandCode"] != 'undefined'){
     ...
     vat.bean.init(function(){
		return "process_object_name=imItemOnHandViewService&process_object_method_name=executeSearchInitial";
   		},{other: true});
  }
...
vat.block.create(vnB_Header = 1, {
	...
	title:"可用庫存查詢作業", rows:[
	 {row_style:"", cols:[
	 {items:[{name:"#F.itemCodeOrEanCode"                  , type:"SELECT"  , init:isItemCodeOrEanCode},
	 ...
	 {items:[{name:"#F.startWarehouseCode"       , type:"SELECT"  ,  bind:"startWarehouseCode", init:allDeliveryWarehouses},
	         {name:"#L.between"                  , type:"LABEL" , value:"至"},
	         {name:"#F.endWarehouseCode"         , type:"SELECT"  ,  bind:"endWarehouseCode", init:allArrivalWarehouses}], td:" colSpan=3"},
	 ...
	 {items:[{name:"#F.taxType"                  , type:"SELECT"  ,  bind:"taxType", init:allTaxRadio}]},
...
function kweImDetail(){
	...
	vat.item.make(vnB_Detail, "itemCode"            , {type:"TEXT" , view:"fixed", size:13, maxLen:13, mode:"READONLY", desc:"品號"});
	...
	vat.item.make(vnB_Detail, "itemBrand"           , {type:"TEXT" , view:"", size: 3, maxLen: 3, mode:"READONLY", desc:"品牌"});
	vat.item.make(vnB_Detail, "itemBrandName"       , {type:"TEXT" , view:"", size: 8, maxLen:12, mode:"READONLY", desc:"品牌名稱"});
...
function loadBeforeAjxService(){
	...
	vat.item.setValueByName("#F.itemCName", itemCName.replace(/^\s+|\s+$/, ''));
	...
	var processString = "process_object_name=imItemOnHandViewService&process_object_method_name=getAJAXSearchPageData" +
