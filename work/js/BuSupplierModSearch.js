...
function headerInitial(){
      var Suptype = [["", "", ""], ["<請選擇>", "國內", "國外"], ["", "1", "2"]];           
	  var CateCode = [["", "", ""], ["<請選擇>", "進貨商", "費用商"], ["", "1", "2"]];   
	vat.block.create( vnB_Header, {
		...
		title:"供應商查詢作業", rows:[  
	 		{row_style:"", cols:[
				...
				{items:[{name:"#L.headId", type:"LABEL", value:"headId"}]},
			 	{items:[{name:"#F.headId", type:"TEXT", bind:"headId", size:20}]}
			 	/*{items:[{name:"#L.supplierTypeCode", type:"LABEL", value:"廠商類別"}]},
			 	{items:[{name:"#F.supplierTypeCode", type:"SELECT", bind:"supplierTypeCode",init:Suptype, size:20}]},
			 	{items:[{name:"#L.categoryCode", type:"LABEL", value:"廠商類型"}]},
			 	{items:[{name:"#F.categoryCode", type:"SELECT", bind:"categoryCode",init:CateCode, size:20}]}*/
			]}
...
function detailInitial(){
  ...
  var vbCanGridModify = true;
	...
	vat.item.make(vnB_Detail, "supplierCode"	, {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"廠商代號"      });	
	//vat.item.make(vnB_Detail, "supplierTypeCode"  , {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"廠商類別"      });
	//vat.item.make(vnB_Detail, "categoryCode"	  , {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"廠商類型"     });
	vat.item.make(vnB_Detail, "headId"	  , {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"headId"     });
...
function loadBeforeAjxService(){
   // alert("loadBeforeAjxService");	
	...
	var processString = "process_object_name=buSupplierModService&process_object_method_name=getAJAXSearchPageData" + 
...
function saveBeforeAjxService() {
	...
	var processString = "";
	if (checkEnableLine()) {
		processString = "process_object_name=buSupplierModService&process_object_method_name=saveSearchResult";
	}
	return processString;
}								
...
//alert("xxxxxxxxxxxxxxxxxxxxxxxxx");
	   vat.block.pageSearch(vnB_Detail, {
    		funcSuccess : 
    		    function(){
    		      vat.block.submit(
    		                     function(){ return "process_object_name=buSupplierModService&process_object_method_name=getSearchSelection";
