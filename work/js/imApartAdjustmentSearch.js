...
function searchInitial(){ 
  ...
  if(typeof document.forms[0]["#loginBrandCode"] != 'undefined'){
     vat.bean().vatBeanOther = 
		...
		vat.bean.init(	
	  		function(){
				return "process_object_name=imGeneralAdjustmentAction&process_object_method_name=performApartSearchInitial"; 
	    	},{								
	    		other: true
    	}); 
  }
...
function headerInitial(){
	...
	var allStatus =  [["","","true"],						 
                 ["暫存", "簽核完成","作廢","結案"],
                 ["SAVE","FINISH","VOID","CLOSE"]];
                 
	var allSourceOrderTypeCode = [["", "", "請選擇"], ["分割 ", "整理"], ["2", "1"]];
	
vat.block.create( vnB_Header, {
	id: "vatBlock_Head", generate: true,
	table:"cellspacing='1' class='default' border='0' cellpadding='2'",
	title:"拆/併貨調整單查詢作業", rows:[  
	 		{row_style:"", cols:[
	 			{items:[{name:"#L.orderTypeCode", 		type:"LABEL"  , value:"單別"}]},
				{items:[{name:"#F.orderTypeCode", 		type:"SELECT" , mode:"READONLY" , init:allOrderTypeCodes , size:1}]},
			 	{items:[{name:"#L.orderNo", 			type:"LABEL"  , value:"單號"}]},
				{items:[{name:"#F.orderNoStart",		type:"TEXT"   , size:10},
						{name:"#L.between", 			type:"LABEL"  , value:" 至 "},
						{name:"#F.orderNoEnd", 			type:"TEXT"   , size:10}]},
			...
			{row_style:"", cols:[
				...
				{items:[{name:"#F.sourceOrderTypeCode",	type:"SELECT", init:allSourceOrderTypeCode, size:10 }]},
				{items:[{name:"#L.adjustmentDate", 		type:"LABEL"  , value:"調整日期"}]},
				{items:[{name:"#F.adjustmentDateStart",	type:"DATE"   , size:10},
						{name:"#L.between", 			type:"LABEL"  , value:" 至 "},
						{name:"#F.adjustmentDateEnd", 	type:"DATE"   , size:10}], td:" colSpan=3"}	
			]}
		], 	 
		beginService:"",
		closeService:""			
...
function detailInitial(){
  ...
  var vbCanGridModify = true;
	...
	vat.item.make(vnB_Detail, "sourceOrderTypeCode"	, {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"調整單別"      });
...
function loadBeforeAjxService(){
   // alert("loadBeforeAjxService");	
	...
	var processString = "process_object_name=imGeneralAdjustmentService&process_object_method_name=getAJAXApartSearchPageData" + 
                      "&loginBrandCode"         + "=" + vat.bean().vatBeanOther.loginBrandCode +     
                      "&orderTypeCode"          + "=" + vat.bean().vatBeanOther.orderTypeCode +     
	                  "&orderNoStart"          	+ "=" + vat.item.getValueByName("#F.orderNoStart"     ) +
...
function saveBeforeAjxService() {
	...
	var processString = "";
	if (checkEnableLine()) {
		processString = "process_object_name=imGeneralAdjustmentService&process_object_method_name=saveApartSearchResult";
	}
	return processString;
}								
...
// 清除
function resetForm(){
	vat.item.setValueByName("#F.orderNoStart", "");
...
function openModifyPage(){
	...
	var page = "/erp/Im_GeneralAdjustment:apartCreate:20091111.page";

	if(!(vFormId == "" || vFormId == "0")){
		var url = page + "?formId=" + vFormId + "&orderTypeCode="+orderTypeCode; 
	
		sc=window.open(url, '拆併貨維護作業', 'menubar=no,resizable=no,scrollbars=no,status=no,resizable=yes,scrollbars=yes');
