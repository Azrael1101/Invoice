...
function kweImBlock(){
 ...
 // kweImInitial()
 //百貨
 if(document.forms[0]["#departmentLogin"   ].value=="01")
 {
 	kweButtonLine01();
...
function checkUploadControl1(){
    //alert('wwww:'+vat.item.getValueByName("#F.customsStatusHidden"));
//alert(tranRecordStatus+" "+orderTypeCode+" "+customsStatus+" "+tranAllowUpload+" "+cStatus+" "+RecordStatus);
	...
	if(status === "FINISH" && ((customsStatus !== "N14" && cStatus === "N" && RecordStatus !== "NF14") || ( cStatus === "E" && tranRecordStatus === "NF14")) && (orderTypeCode == "WMF" || orderTypeCode == "RWD"|| orderTypeCode == "RDW")){
		vat.item.setStyleByName("#B.sendCustoms","display","inline");
		vat.item.setStyleByName("#B.sendBack","display","inline");
	}else if(status === "FINISH" && customsStatus == "N23"){
...
function kweImInitial(){
  if(typeof document.forms[0]["#loginBrandCode"] != 'undefined'){
   // alert(vat.item.getValueByName("#F.orderTypeCode") == "" ? document.forms[0]["#orderTypeCode"].value:vat.item.getValueByName("#F.orderTypeCode"));
    ...
    vat.item.SelectBind(vat.bean("allFixSuppliers"),{ itemName : "#F.allFixSuppliers" }); // 轉入店別
	 ...
	 if("T2" != vat.item.getValueByName("#F.brandCode")){ // 轉入店別只提供T2
	 	vat.item.setAttributeByName("#F.arrivalStoreCode" , "disabled", true);
	 	vat.item.setAttributeByName("#F.allFixSuppliers" , "disabled", true);
	 }
  ...
  //alert("eee");
  if(vat.item.getValueByName("#F.orderTypeCode")||document.forms[0]["#orderTypeCode"].value=='WMF'){
	 ...
	 {row_style:"", cols:[
	 	{items:[{name:"#B.new"         , type:"IMG"    , value:"新增",  src:"./images/button_create.gif", eClick:"createNewForm()"},
				...
				{name:"#B.message"     , type:"IMG"    ,value:"訊息提示",   src:"./images/button_message_prompt.gif", eClick:'showMessage()'},
	 			...
	 			//{name:"SPACE"          , type:"LABEL"  ,value:"　"},
	 			{name:"#B.updateApplicationNo"        , type:"IMG"    ,value:"重取申請書號碼",src:"./images/updateApplicationNo.jpg"    , eClick:'updateApplicationNo("IM")'},
				...
				//{name:"SPACE"          , type:"LABEL"  ,value:"　"} ,		 
	 			...
	 			{name:"SPACE",			type:"LABEL", value:"　"},
	 			{name:"SPACE",			type:"LABEL", value:"　"},
	 			{name:"#B.print1"       , type:"IMG"    ,value:"POS領用單",   src:"./images/posReceive.jpg" , eClick:'openDiversityWindowForTest("")'}
	 			],
...
var vsRowStyle= vat.bean("loginBrandCode").indexOf("T2") > -1 ? "" : " style= 'display:none;'";
	...
	title:"調撥單維護作業", rows:[
	 ...
	 {items:[{name:"#F.remark1"     , type:"TEXT",   bind:"remark1", size:80, maxLen:200, desc:"放一般備註內容"}], td:" colSpan=3"},
...
vat.block.create(vnB_master, {
	...
	rows:[
	 ...
	 {items:[{name:"#F.arrivalContactPerson"    , type:"TEXT"  , bind:"arrivalContactPerson", size:12, mode:"READONLY"},
	 		 {name:"#F.arrivalContactPersonName", type:"TEXT"  , bind:"arrivalContactPersonName", size:12, mode:"READONLY"}]}]},
	 {row_style:"", cols:[
	 {items:[{name:"#L.arrivalStoreCode"    	, type:"LABEL" , value:"轉入店別"}]},
	 {items:[{name:"#F.arrivalStoreCode"    	, type:"SELECT", bind:"arrivalStoreCode", size:12, eChange:'changeWarehouseCode("arrival")'},
			 {name:"#F.customsArrivalStoreCode" , type:"TEXT"  , bind:"customsArrivalStoreCode", size:1, mode:"READONLY"}], td:" colSpan=2"},
	 {items:[{name:"#L.arrivalStoreCode"    	, type:"LABEL" , value:"維修廠商"}]},
	 {items:[{name:"#F.allFixSuppliers"    	, type:"SELECT", bind:"rpSupplier", size:12, eChange:'getFixAddr()'}], td:" colSpan=2"}]},
	 {row_style:"", cols:[
...
//  }
	...
	vat.item.make(vnB_Detail, "unit"                      , {type:"TEXT"  , desc:"單位",mode:"HIDDEN"});
...
//var vItemCode  = vat.item.getGridValueByName("itemCode" , vLineId);
	var processString = "";
	//alert("saveBeforeAjxService");
	if (checkEnableLine()) {
		processString = "process_object_name=imMovementMainService&process_object_method_name=updateAJAXPageLinesData" +
		"&headId=" + vat.item.getValueByName("#F.headId") + "&status=" + vat.item.getValueByName("#F.status")+"&employeeCode="+  vat.bean().vatBeanOther.loginEmployeeCode+
		                    "&brandCode=" + vat.item.getValueByName("#F.brandCode")+"&orderTypeCode=" + vat.item.getValueByName("#F.orderTypeCode");//by jason 修正連續建單gooddesc、spcdesc無法寫入
		//alert(processString);
	}
...
function openDiversityWindowForTest(){
   
   url = "http://10.1.94.161:8080/crystal/t2/IM0168.rpt?prompt0=&prompt1="+vat.item.getValueByName("#F.orderTypeCode")+"&prompt2=&prompt3=&prompt4="+ vat.item.getValueByName("#F.orderNo")+"&prompt5="+ vat.item.getValueByName("#F.orderNo");
   window.open(url,'CC', 'menubar=yes,resizable=yes,scrollbars=yes,status=yes,width=1024,height=768,left=0,top=0');
   //return url;
  
   /*
   url = "http://10.1.94.161:8080/napreport/t2/IM0168.rpt?prompt0=&prompt1="+vat.item.getValueByName("#F.orderTypeCode")+"&prompt2=&prompt3=&prompt4="+ vat.item.getValueByName("#F.orderNo")+"&prompt5="+ vat.item.getValueByName("#F.orderNo");
   window.open(url,'CC', 'menubar=yes,resizable=yes,scrollbars=yes,status=yes,width=1024,height=768,left=0,top=0');
   //return url;
  */
}
...
function doSubmit(formAction){
	...
	if(confirm(alertMessage)){
		...
		var vsArrivalDate = vat.item.getValueByName("#F.arrivalDate");
		var vsAllowSubmit = true;
	    ...
	    vat.block.pageDataSave(vnB_Detail,
		    {  funcSuccess:function(){
				...
				if("SUBMIT" == formAction || "SUBMIT_BG" == formAction){
					...
					if(null == vat.item.getValueByName("#F.arrivalDate")){
							alert("轉入日期未輸入");
							vsAllowSubmit = false;
							}
					if(vat.item.getValueByName("#F.deliveryWarehouseCode")== vat.item.getValueByName("#F.arrivalWarehouseCode")){
...
function importFormData(){
    ...
    var beanName = "";
    var suffix ="";
	if( vat.item.getValueByName("#F.brandCode").indexOf("T2") > -1  && vsOrderTypeCode == "KMF"){
		suffix =
		"&importBeanName="+ "T2_IM_MOVEMENT_OFFSHORE_ITEM" +
		"&importFileType=XLS" +
        "&processObjectName=imMovementMainService" +
        "&processObjectMethodName=executeImportMovementItems" +
        "&arguments=" +vat.item.getValueByName("#F.headId") + "{$}" + 
        			   vat.item.getValueByName("#F.deliveryWarehouseCode") + "{$}" + 
        			   vat.item.getValueByName("#F.arrivalWarehouseCode") +
        "&parameterTypes=LONG{$}STRING{$}STRING" +
        "&blockId=" + vnB_Detail
	}else{
		if( vat.item.getValueByName("#F.brandCode").indexOf("T2") > -1 ){
			beanName = vat.bean().vatBeanOther.isThreePointMoving ?"T2_IM_MOVEMENT_ITEM_3":"T2_IM_MOVEMENT_ITEM_2";
				 ...
				 if(vsFormStatus == "WAIT_OUT" || vsFormStatus == "WAIT_IN" || vsFormStatus == "SIGNING" ||
		 			...
		 			if("Y" == vsNotInProcessVoidMode && (vat.item.getValueByName("#F.cmMovementNo") == "" || vat.item.getValueByName("#F.cmMovementNo") == null ) ){
						if((vsFormStatus == "WAIT_IN" && vat.item.getValueByName("#F.customsDeliveryWarehouseCode") != vat.item.getValueByName("#F.customsArrivalWarehouseCode"))){
							...
							/*if( vsOrderTypeCode == "WWF" ||  vsOrderTypeCode == "WWP"){//WWF開作廢鈕
								vat.item.setStyleByName("#B.void" , "display", "inline");
							}else{*/
								vat.item.setStyleByName("#B.void" , "display", "none");
							//}
...
function changeDeclarationStyle(){
	var vsType="";
     ...
     if(vat.item.getValueByName("#F.orderTypeCode")!="WMF" && vat.item.getValueByName("#F.orderTypeCode")!="MDF") {
        if("P" == vat.item.getValueByName("#F.taxType") ||
			vat.item.getValueByName("#F.customsDeliveryWarehouseCode") == vat.item.getValueByName("#F.customsArrivalWarehouseCode")){
			//alert(vat.item.getValueByName("#F.customsDeliveryWarehouseCode")+"   /   "+vat.item.getValueByName("#F.customsArrivalWarehouseCode"));
			vsType="none";
		}
     }else{
   		//alert(vat.item.getValueByName("#F.customsDeliveryWarehouseCode")+"   /   "+vat.item.getValueByName("#F.customsArrivalWarehouseCode"));
...
function importMovePrint(){ 
	...
	vat.ajax.XHRequest(
	{
		...
		find: function posPrint(oXHR){
			...
			var WarehouseArrivalName	 = "èª¿å¥éå¸ï¼" + vat.ajax.getValue("WarehouseArrivalName", oXHR.responseText) + "\n";
			var WarehouseDeliveryName	 = "èª¿åºéå¸ï¼" + vat.ajax.getValue("WarehouseDeliveryName", oXHR.responseText) + "\n";
			var ItemCountLabel           = "ååç¸½æ¸ï¼";
			var ItemCount                = vat.ajax.getValue("ItemCount", oXHR.responseText);
			var DeliveryDateLabel        = "æ¥æï¼";
			var DeliveryDate             = vat.ajax.getValue("DeliveryDate", oXHR.responseText);
			var OrderNoLabel             = "å®æèç¢¼ï¼";
			var OrderNo                  = vat.ajax.getValue("OrderNo", oXHR.responseText);
			...
			var ItemPrices               = vat.ajax.getValue("ItemPrices", oXHR.responseText);
			var ItemPricesArray          = ItemPrices.split(",");
			var Quantity                 = vat.ajax.getValue("Quantity", oXHR.responseText);
			var QuantityArray            = Quantity.split(",");
			var ItemNames                = vat.ajax.getValue("ItemNames", oXHR.responseText);
			...
			for(var i = 0; i < times; i++){
				...
				printTxtKeep = keepTypes[i];
				document.getElementById("clsInvoice").Align(0);
				document.getElementById("clsInvoice").PrintTextUseEncoding("\n", 0, 0, 0,"Big5");
				document.getElementById("clsInvoice").PrintTextUseEncoding(printTxtKeep, 0, 0, 0,"Big5");
				document.getElementById("clsInvoice").PrintTextUseEncoding("\n", 0, 0, 0,"Big5");
				
				printTxtHead = printTxtHead + Company+ Title;
				printTxtHead = printTxtHead + WarehouseDeliveryName;
				...
				space = "";
				while (devideLine.length - (OrderNoLabel.length * 2) - OrderNo.length - DeliveryDate.length - (DeliveryDateLabel.length * 2) - space.length > 0){
						space = space + " ";
				}
				printTxt = printTxt + devideLine + "\n";
				printTxt = printTxt + OrderNoLabel + OrderNo + space + DeliveryDateLabel + DeliveryDate + "\n";
				printTxt = printTxt  + WarehouseArrivalName;
				printTxt = printTxt  + "-------------------- ---------------------- ----" + "\n";
				
				for (j=0; j < ItemIdArray.length; j++){
					while(ItemIdArray[j].length < 20){
						ItemIdArray[j] = ItemIdArray[j] + " ";
					}
					while(UnitPriceArray[j].length < 22){
						UnitPriceArray[j] = " " + UnitPriceArray[j];
					}
					while(QuantityArray[j].length < 4){
						QuantityArray[j] = " " + QuantityArray[j];
					}
					printTxt = printTxt + ItemIdArray[j] + " " + UnitPriceArray[j] + " " + QuantityArray[j] + "\n";
					
					printTxt = printTxt + " " + ItemNamesArray[j] + "\n";
				}
				
				printTxt = printTxt + "------------------------------------------- ----";
				
				space = "";
				while (devideLine.length - (ItemCountLabel.length * 2) - ItemCount.length - space.length > 0){
						space = space + " ";
				}
				printTxt = printTxt + ItemCountLabel + space + ItemCount + "\n";
				printTxt = printTxt + devideLine + "\n";
				printTxt = printTxt + "\n";
				printTxt = printTxt + "è½åºäººå¡ç°½åï¼_____________________________" +"\n";
				printTxt = printTxt + "\n";
				printTxt = printTxt + "æ¶è²¨äººå¡ç°½åï¼_____________________________" +"\n";
				printTxt = printTxt + "\n";
				
				if(i < times-1){
					printTxt = printTxt + "vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv" +"\n";
					printTxt = printTxt + "vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv" +"\n";
					printTxt = printTxt + "vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv" +"\n";
				}
				printTxt = printTxt + "\n";
...
function importMovePrint(){ 
	...
	vat.ajax.XHRequest(
	{
		...
		find: function posPrint(oXHR){
			...
			var PrinterId		         = vat.ajax.getValue("PrinterId", oXHR.responseText);
			var Company			         = vat.ajax.getValue("CompanyName", oXHR.responseText) + "\n";
			var WarehouseArrivalName	 = "調入門市：" + vat.ajax.getValue("WarehouseArrivalName", oXHR.responseText) + "\n";
			var WarehouseDeliveryName	 = "調出門市：" + vat.ajax.getValue("WarehouseDeliveryName", oXHR.responseText) + "\n";
			var ItemCountLabel           = "商品總數：";
			var ItemCount                = vat.ajax.getValue("ItemCount", oXHR.responseText);
			var DeliveryDateLabel        = "日期：";
			var DeliveryDate             = vat.ajax.getValue("DeliveryDate", oXHR.responseText);
			var OrderNoLabel             = "單據號碼：";
			var OrderNo                  = vat.ajax.getValue("OrderNo", oXHR.responseText);
			var Title                    = vat.ajax.getValue("Title", oXHR.responseText) + "\n";
			...
			for(var i = 0; i < times; i++){
				...
				printTxt = printTxt + devideLine + "\n";
				printTxt = printTxt + "\n";
				printTxt = printTxt + "轉出人員簽名：_____________________________" +"\n";
				printTxt = printTxt + "\n";
				printTxt = printTxt + "收貨人員簽名：_____________________________" +"\n";
				printTxt = printTxt + "\n";
