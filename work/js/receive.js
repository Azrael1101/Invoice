			...
			if (vat.ajax.handleState()) {
				if (vat.ajax.found(vat.ajax.xmlHttp.responseText)) {
					...
					if(vat.ajax.getValue("ItemCName", vat.ajax.xmlHttp.responseText) == '查無資料'){
						var i = parseInt(nItemLine) - 1;
						var text = 'input[@id*=A#Y' + i +'#X2]';
						$(text).css("color", "red").css("font-size","16px");						
					}
				}
			}
		});
...
function loadSuccessAfter() {
	setTimeout ( "setDefaultInvoice()" , 300 );
}
...
function setDefaultInvoice(){
	for(index = vat.formD.page[0].firstIndex ; index < vat.formD.page[0].firstIndex + 10 ; index++){
		if( "" == vat.formD.itemDataGet(index,"invoiceNo") ){ 
			vat.formD.itemDataBind(0,index,"invoiceNo", document.forms[0]["#defaultInvoiceNo"].value ) ;  
		}		
	}
