	...
	title:"", rows:[  
	 {row_style:"", cols:[
	 	{items:[{name:"#B.search"      , type:"IMG"      , value:"查詢",  src:"./images/button_find.gif", eClick:"doSearch()"},
	 			...
	 			{name:"SPACE"          , type:"LABEL"    , value:"　"},
	 			{name:"#B.update"      , type:"IMG"      , value:"檢視",   src:"./images/button_view.gif"  , eClick:"doClosePickerForPos()"}
...
function detailInitial(){
  ...
  var vbCanGridModify = true;
	...
	vat.block.pageLayout(vnB_Detail, {
														...
														indicate            : function(){openSalesOrderSearch()}
														});
...
// 檢視按下後的動作
function doClosePickerForPos(){
	   vat.block.pageSearch(vnB_Detail, {
    		funcSuccess : 
    		    function(){
    		      vat.block.submit(
    		                     function(){ 
    		                     	return "process_object_name=buAddressBookAction&process_object_method_name=performSearchCustomerSelection";
    		                     }, { bind:true, link:false, other:false, picker:true, isPicker:true, blockId:vnB_Detail,funcSuccess:function() {
    			 	var customerResult = vat.bean().vatBeanPicker.customerResult[0].customerCode;
    			 	window.close();
...
function openSalesOrderSearch(){
	var nItemLine = vat.item.getGridLine();
    var vCustomerCode = vat.item.getGridValueByName("customerCode", nItemLine);
	var url = "/erp/SO_Department_Order:main:salesSearch20190108.page?customerCode="+vCustomerCode
	var result = window.showModalDialog(url, '', 'dialogWidth:1280px;dialogHeight:760px;dialogTop:0px;dialogLeft:0px;status:no;');
}