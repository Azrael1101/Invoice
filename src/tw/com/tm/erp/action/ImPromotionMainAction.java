...
public class ImPromotionMainAction {
    
    private static final Log log = LogFactory.getLog(ImPromotionMainAction.class);
	...
	try{	
	    ...
	    returnMap = imPromotionMainService.executeInitial(parameterMap);
	}catch (Exception ex) {
	...
	try{
	    ...
	    Map resultMap = imPromotionMainService.updateAJAXPromotion(parameterMap);
    ...
    private void executeProcess(Object otherBean, Map resultMap, ImPromotion promotionBean, MessageBox msgBox) 
        ...
        try{
	        ...
	        if(msgBox != null)
                ...
                ImPromotionMainService.startProcess(promotionBean);
            }else{
	        if(msgBox != null)
	        ...
	        if(!OrderStatus.SAVE.equals(formStatus) || (OrderStatus.REJECT.equals(beforeChangeStatus)
	            ...
	            if(StringUtils.hasText(assignmentId)){
	                ...
	                Object[] processInfo = ImPromotionMainService.completeAssignment(assignId, result);
            ...
            siProgramLogAction.createProgramLog(ImPromotionMainService.PROGRAM_ID, MessageStatus.LOG_ERROR, identification, message, promotionBean.getLastUpdatedBy());
            log.error(message);
        }
    }
