...
public class ImPromotionAction {
    
    private static final Log log = LogFactory.getLog(ImPromotionAction.class);
    
    private ImPromotionService imPromotionService;
    ...
    public void setImPromotionService(ImPromotionService imPromotionService) {
	this.imPromotionService = imPromotionService;
    }
	...
	try{
	    Object formLinkBean = parameterMap.get("vatBeanFormLink");
    	    ...
    	    Long headId = imPromotionService.getPromotionHeadId(formLinkBean); 
	    ...
	    Map resultMap = imPromotionService.updateAJAXPromotion(parameterMap);
	...
	MessageBox msgBox = new MessageBox();
	try{
	    Map resultMap = imPromotionService.updatePromotionWithActualOrderNO(parameterMap);
	    ImPromotion promotionBean = (ImPromotion)resultMap.get("entityBean");
	    msgBox.setMessage((String)resultMap.get("resultMsg") + "是否繼續新增？");
	    Object otherBean = parameterMap.get("vatBeanOther");
	...
	try{
		...
		if(msgBox != null)
	        ...
	        ImPromotionService.startProcess(promotionBean);
	    }else{
		if(msgBox != null)
		...
		if(!OrderStatus.SAVE.equals(formStatus) || (OrderStatus.REJECT.equals(beforeChangeStatus)
		    ...
		    if(StringUtils.hasText(assignmentId)){
		        ...
		        Object[] processInfo = ImPromotionService.completeAssignment(assignId, result);
	    ...
	    siProgramLogAction.createProgramLog(ImPromotionService.PROGRAM_ID, MessageStatus.LOG_ERROR, identification, message, promotionBean.getLastUpdatedBy());
	    log.error(message);
	}
    }
	...
	try{
	    Object formLinkBean = parameterMap.get("vatBeanFormLink");
            Object otherBean = parameterMap.get("vatBeanOther");
            Long headId = imPromotionService.getPromotionHeadId(formLinkBean);
            promotionBean = imPromotionService.findById(headId);
            String employeeCode = (String)PropertyUtils.getProperty(otherBean, "employeeCode");	    	    
	    try{      	     	       
		promotionBean = imPromotionItemService.saveCheckedPromotionData(headId, parameterMap, ImPromotionService.PROGRAM_ID, employeeCode);
	    }catch(Exception ex){
		if(promotionBean != null){
		    identification = MessageStatus.getIdentification(promotionBean.getBrandCode(), 
			    promotionBean.getOrderTypeCode(), promotionBean.getOrderNo());
		    message = ex.getMessage();
		    siProgramLogAction.createProgramLog(ImPromotionService.PROGRAM_ID, MessageStatus.LOG_ERROR, 
