...
public class SoSalesOrderReturnMainAction {
    
    private static final Log log = LogFactory.getLog(SoSalesOrderReturnMainAction.class);
	...
	public List<Properties> performTransaction(Map parameterMap){
		...
		try{
			...
			ImDeliveryHead imDeliveryHead = imDeliveryMainService.findImDeliveryHeadById(headId);

			//for 儲位用
			if(imStorageAction.isStorageExecute(imDeliveryHead)){
				//取單號後，扣庫存前，執行更新儲位單頭與單身，比對單據明細與儲位明細
				executeStorage(imDeliveryHead);
			}
			...
			if(StringUtils.hasText(processId) || !OrderStatus.FINISH.equals(formStatus))
	...
	if(isStartProcess){
		if(OrderStatus.UNCONFIRMED.equals(status)){
            msgBox.setMessage(identification + "表單已反確認！");
    ...
    private void executeProcess(Object otherBean, Map resultMap, ImDeliveryHead deliveryHeadBean, MessageBox msgBox) 
        ...
        try{
        	...
        	if(msgBox != null)
        		...
        		if("T2".equals(deliveryHeadBean.getBrandCode())
        				|| "T6CO".equals(deliveryHeadBean.getBrandCode()))
        			ImDeliveryMainService.startProcessForReturnT2(deliveryHeadBean);
        		else
    	    	...
    	    	if(!OrderStatus.SAVE.equals(formStatus) || 
    	    			(OrderStatus.REJECT.equals(beforeChangeStatus) && OrderStatus.SAVE.equals(formStatus))){
