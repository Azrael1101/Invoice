...
public class ImPromotionReCombineMainAction {

	private static final Log log = LogFactory
			.getLog(ImPromotionMainAction.class);

	private ImPromotionMainService imPromotionMainService;

	private ImPromotionItemService imPromotionItemService;

	private ImPromotionReCombineMainService imPromotionReCombineMainService;
	...
	public void setImPromotionReCombineMainService(
			ImPromotionReCombineMainService imPromotionReCombineMainService) {
		this.imPromotionReCombineMainService = imPromotionReCombineMainService;
	}
	...
	public List<Properties> performInitial(Map parameterMap) {
		...
		try {
			...
			returnMap = imPromotionReCombineMainService
					.executeInitial(parameterMap);
		} catch (Exception ex) {
	...
	private void validateInitialRequiredParameters(Map parameterMap,
			boolean isDirectSearch) throws Exception {
	...
	private String getApprovalResult(String processId,
			String beforeChangeStatus, String formStatus) {

		String approvalResult = "送出";
		if (StringUtils.hasText(processId)) {
			if (OrderStatus.SAVE.equals(formStatus)
					|| OrderStatus.VOID.equals(formStatus)
	...
	public List<Properties> performTransaction(Map parameterMap) {
		...
		try {
			...
			Object otherBean = parameterMap.get("vatBeanOther");
			Long headId = imPromotionReCombineMainService
					.getPromotionHeadId(formLinkBean);
			...
			if (OrderStatus.SAVE.equals(beforeChangeStatus)
					&& OrderStatus.SIGNING.equals(formStatus)) {
				imPromotionItemService.saveActualOrderNo(headId, employeeCode);
			}
			Map resultMap = imPromotionReCombineMainService
					.updateAJAXPromotion(parameterMap);
			ImPromotion promotionBean = (ImPromotion) resultMap
					.get("entityBean");
	...
	public List<Properties> getOrderNo(Map parameterMap) {
		...
		MessageBox msgBox = new MessageBox();
		try {
			Map resultMap = imPromotionMainService
					.updatePromotionWithActualOrderNO(parameterMap);
			ImPromotion promotionBean = (ImPromotion) resultMap
					.get("entityBean");
			msgBox.setMessage((String) resultMap.get("resultMsg") + "是否繼續新增？");
			Object otherBean = parameterMap.get("vatBeanOther");
	...
	private void wrappedMessageBox(MessageBox msgBox,
			ImPromotion promotionBean, boolean isStartProcess,
		...
		if (isStartProcess) {
			...
			if (OrderStatus.SIGNING.equals(status) || isExecFunction) {
				...
				msgBox.setMessage(identification + "表單已"
						+ OrderStatus.getChineseWord(status) + "！");
			}
	...
	private void executeProcess(Object otherBean, Map resultMap,
			ImPromotion promotionBean, MessageBox msgBox) throws Exception {

		String message = null;
		String identification = MessageStatus.getIdentification(promotionBean
				.getBrandCode(), promotionBean.getOrderTypeCode(),
				promotionBean.getOrderNo());
		try {
			String processId = (String) PropertyUtils.getProperty(otherBean,
					"processId");
			String assignmentId = (String) PropertyUtils.getProperty(otherBean,
					"assignmentId");
			String approvalResult = (String) PropertyUtils.getProperty(
					otherBean, "approvalResult");
			String approvalComment = (String) PropertyUtils.getProperty(
					otherBean, "approvalComment");
			String beforeChangeStatus = (String) PropertyUtils.getProperty(
					otherBean, "beforeChangeStatus");
			String formStatus = (String) PropertyUtils.getProperty(otherBean,
					"formStatus");
			String approvalResultMsg = getApprovalResult(processId,
					beforeChangeStatus, formStatus);

			if (!StringUtils.hasText(processId) || "0".equals(processId)) {
				if (msgBox != null)
					wrappedMessageBox(msgBox, promotionBean, true, false);
				ImPromotionReCombineMainService.startProcess(promotionBean);
			} else {
				if (msgBox != null)
					wrappedMessageBox(msgBox, promotionBean, false, false);
				if (!OrderStatus.SAVE.equals(formStatus)
						|| (OrderStatus.REJECT.equals(beforeChangeStatus) && OrderStatus.SAVE
								.equals(formStatus))) {
					if (StringUtils.hasText(assignmentId)) {
						...
						Object[] processInfo = ImPromotionReCombineMainService
			...
			siProgramLogAction.createProgramLog(
					ImPromotionReCombineMainService.PROGRAM_ID,
					MessageStatus.LOG_ERROR, identification, message,
					promotionBean.getLastUpdatedBy());
			log.error(message);
		}
	}
	...
	public List<Properties> performTransactionForBackGround(Map parameterMap) {
		...
		try {
			Object formLinkBean = parameterMap.get("vatBeanFormLink");
			Object otherBean = parameterMap.get("vatBeanOther");
			Long headId = imPromotionMainService
					.getPromotionHeadId(formLinkBean);
			promotionBean = imPromotionMainService.findById(headId);
			String employeeCode = (String) PropertyUtils.getProperty(otherBean,
					"loginEmployeeCode");
			try {
				promotionBean = imPromotionItemService
						.saveCheckedPromotionData(headId, parameterMap,
								ImPromotionMainService.PROGRAM_ID, employeeCode);
			} catch (Exception ex) {
				if (promotionBean != null) {
					identification = MessageStatus.getIdentification(
							promotionBean.getBrandCode(), promotionBean
									.getOrderTypeCode(), promotionBean
									.getOrderNo());
					message = ex.getMessage();
					siProgramLogAction.createProgramLog(
							ImPromotionMainService.PROGRAM_ID,
							MessageStatus.LOG_ERROR, identification, message,
							promotionBean.getLastUpdatedBy());
				}
			}
	...
	public List<Properties> performSearchInitial(Map parameterMap) {
		...
		try {
			Object otherBean = parameterMap.get("vatBeanOther");
			String vatPickerId = (String) PropertyUtils.getProperty(otherBean,
					"vatPickerId");
			...
			returnMap = imPromotionReCombineMainService
					.executeSearchInitial(parameterMap);
		} catch (Exception ex) {
	...
	public List<Properties> performSearchSelection(Map parameterMap) {
		...
		try {
			returnMap = imPromotionReCombineMainService
					.getSearchSelection(parameterMap);
		} catch (Exception ex) {
			MessageBox msgBox = new MessageBox();
			msgBox.setMessage(ex.getMessage());
			returnMap = new HashMap();
	...
	public List<Properties> performCombineSearchInitial(Map parameterMap) {

		Map returnMap = null;
		try {
			returnMap = imPromotionReCombineMainService.executeCombineSearchInitial(parameterMap);
		} catch (Exception ex) {
			System.out.println("執行促銷活動查詢初始化時發生錯誤，原因：" + ex.toString());
			MessageBox msgBox = new MessageBox();
			msgBox.setMessage(ex.getMessage());
	...
	public List<Properties> performSearchCombineSelection(Map parameterMap) {

		Map returnMap = null;
		try {
			returnMap = imPromotionReCombineMainService
					.updateSearchCombineSelection(parameterMap);
		} catch (Exception ex) {
			MessageBox msgBox = new MessageBox();
			msgBox.setMessage(ex.getMessage());
			returnMap = new HashMap();
