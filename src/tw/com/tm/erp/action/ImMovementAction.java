...
public class ImMovementAction {

    private static final Log log = LogFactory.getLog(ImMovementAction.class);
	...
	public List<Properties> getOrderNo(Map parameterMap) {
		...
		try {
			Map resultMap = imMovementService.updateImMovementWithActualOrderNO(parameterMap);
			...
			if (!StringUtils.hasText(processId) || "0".equals(processId)) {
				wrappedMessageBox(msgBox, imMovementHead, true, true);
			} else {
				wrappedMessageBox(msgBox, imMovementHead, false, true);
			}
		} catch (Exception ex) {
	...
	public List<Properties> performTransaction(Map parameterMap) {
		...
		try {
			...
			Object otherBean = parameterMap.get("vatBeanOther");
			Long headId = imMovementService.getImMovementHeadId(formLinkBean);
			...
			ImMovementHead imMovementHead = imMovementService.findById(headId);
			// 確認是否允許流程，不允許會丟例外
			...
			if (OrderStatus.SAVE.equals(beforeChangeStatus) || OrderStatus.WAIT_IN.equals(beforeChangeStatus)
				...
				imMovementService.saveActualOrderNo(headId, employeeCode);
			}
			
			//先更新單頭 回傳BEAN給儲位單頭更新
			resultMap = imMovementService.updateAJAXMovementParcial(parameterMap);
			...
			resultMap = imMovementService.updateAJAXMovement(parameterMap);
			
			imMovementHead = (ImMovementHead) resultMap.get("entityBean");
			
			imMovementService.updateItemIndex(imMovementHead); // 重新排序INDEX_NO
			...
			// 最後在更新一次 proccessId
			imMovementService.updateProcessId(imMovementHead.getHeadId(), imMovementHead.getProcessId() == null ? (Long)resultMap
	 ...
	 */
	private void wrappedMessageBox(MessageBox msgBox, ImMovementHead head, boolean isStartProcess, boolean isExecFunction) {
    ...
    public List<Properties> performTransactionForBackGround(Map parameterMap) {
		...
		try {
			...
			try {
				imMovement = imMovementService.findById(headId);
				...
				List errorMsgs = imMovementService.updateCheckedImMovementData(parameterMap);
				if (errorMsgs.size() == 0) {
				...
				if (imMovement != null) {
					...
					imMovementService.updateProcessId(imMovement.getHeadId(), imMovement.getProcessId());
				}
...
//		}
	...
	private void executeProcess(Object otherBean, Map resultMap, ImMovementHead imMovement, MessageBox msgBox)
		...
		try {
			...
			if (!StringUtils.hasText(processId) || "0".equals(processId)) {
				if (msgBox != null)
					wrappedMessageBox(msgBox, imMovement, "Y".equals(updateForm) ? false : true, false);

				Object[] processObj = ImMovementService.startProcess(imMovement); // 起流程後，取回流程的processId
				...
				if (StringUtils.hasText(assignmentId)) {
					...
					ceapProcessService.updateProcessSubject(Long.valueOf(processId), imMovementService.getProcessSubject(imMovement));
					Object[] processInfo = ImMovementService.completeAssignment(assignId, approvalResult);
	...
	public List<Properties> voidMovement(Map parameterMap) {
		...
		try {
			...
			Map resultMap = imMovementService.updateImMovementToVoid(headId, loginEmployeeCode, action);
	...
	public List<Properties> modifyDeliveryDate(Map parameterMap) {
		...
		log.info("1.performTransaction");
		try {
			HashMap resultMap = imMovementService.updateDate(parameterMap);
	...
	public int autoCreateImMovement(String brandCode, String orderTypeCode, String orderNo, Date date) throws Exception {
		...
		MessageBox msgBox = new MessageBox();
		try {
			ImMovementHead imMovementHead = imMovementService
			...
			if (imMovementHead == null)
				...
				newImMovementHead = imMovementService.createNewImMovement(imMovementHead, date);
				if (newImMovementHead != null){
					log.info("========== 自動起調撥單流程 ==========");
					ImMovementService.startProcess(newImMovementHead);
				}else
	...
	public List<Properties> performCopyTransaction(Map parameterMap) {
		...
		try {
			...
			for (int i = 0; i < result.size(); i++) {
				...
				if (imMovementHead != null) {
					ImMovementHead newImMovementHead = imMovementService.createToNewImMovement(imMovementHead, parameterMap);
					if (newImMovementHead != null) {
						ImMovementService.startProcess(newImMovementHead); // 調撥單起流程
						if (sb.length() == 0)
	 ...
	 * 執行儲位單 更新 與 比對
	 */
	public void executeStorage(ImMovementHead imMovementHead) throws StorageException {
