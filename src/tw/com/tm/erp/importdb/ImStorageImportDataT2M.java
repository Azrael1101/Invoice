...
public class ImStorageImportDataT2M implements ImportDataAbs {
    private static final Log log = LogFactory.getLog(ImStorageImportDataT2M.class);
	...
	imInfo.addFieldName("sourceOrderNo");
	...
	imInfo.setFieldType("sourceOrderNo", "java.lang.String");
	...
	try {
		...
		//entityBean.setDeliveryWarehouseCode(arrivalWarehouseCode);

		String deliveryStorageCode = entityBean.getReserve1();
		if(!StringUtils.hasText(deliveryStorageCode))
		    deliveryStorageCode = ImStorageService.STORAGE_CODE_DEFAULT;
		...
		for (Iterator iterator = imStorageItems.iterator(); iterator.hasNext();) {
		    ...
		    //imStorageItem.setDeliveryStorageCode(deliveryStorageCode);

		    imStorageItem.setArrivalWarehouseCode(arrivalWarehouseCode);
		    //imStorageItem.setDeliveryWarehouseCode(arrivalWarehouseCode);
		    ...
		    if(imStorageItem.getItemCode().endsWith("F")){
			isTax = "F";
		    }else{
			isTax = "P";
		    }
		...
		if("F".equals(isTax)){
		    entityBean.setSourceOrderTypeCode("EIF");
		}else{
		    entityBean.setSourceOrderTypeCode("EIP");
		}

		imStorageHeads.add(entityBean);
	    }

	    List<ImStorageHead> newImStorageHeads = imStorageService.executeBatchImportT2M(imStorageHeads);
