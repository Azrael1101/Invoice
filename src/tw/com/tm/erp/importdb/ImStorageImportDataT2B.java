...
public class ImStorageImportDataT2B implements ImportDataAbs {
    private static final Log log = LogFactory.getLog(ImStorageImportDataT2B.class);
	...
	try {
		...
		for (Iterator iterator = imStorageItems.iterator(); iterator.hasNext();) {
		    ...
		    ImItem imItem = imItemDAO.findImItem(entityBean.getBrandCode(), imStorageItem.getItemCode(), "Y");
		    if(null != imItem){
			imStorageItem.setStorageQuantity(imStorageItem.getStorageQuantity() * imItem.getBoxCapacity());
		    }
