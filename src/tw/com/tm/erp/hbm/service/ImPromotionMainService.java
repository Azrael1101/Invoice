...
public class ImPromotionMainService {
	...
	"indexNo", 		"itemCode", 			"category02",
	"category02Name", 	"itemName", 			"standardPurchaseCost", 
	"originalPrice", 	"margenBeforeDisc", 	"discountAmount", 	"discountPercentage", 
	"totalDiscountAmount",	"margenAfterDisc", 	"totalDiscountPercentage", 	"quantity", 	"stockOnHandQty",
	"reserve5", 		"lineId", 			"isLockRecord", 
	...
	"beginDate", "endDate", "statusName","originalPrice", "totalDiscountAmount", "totalDiscountPercentage", "headId"};
    ...
    public void setImPromotionReCombineDAO(ImPromotionReCombineDAO imPromotionReCombineDAO) {
	...
	try {
	    ...
	    if (form != null) {
		if(OrderStatus.SAVE.equals(form.getStatus()) || OrderStatus.REJECT.equals(form.getStatus())){
		    getVipTypeInfomation(form.getImPromotionCustomers(), form.getBrandCode(), false);
     ...
     * @param isNew
     */
    private void getVipTypeInfomation(List<ImPromotionCustomer> imPromotionCustomers, String brandCode, boolean isNew){

	List<BuCommonPhraseLine> allVIPType = buCommonPhraseService.findCommonPhraseLineByAttribute("VIPType", "model.attribute1='" + brandCode + "'");
	if (allVIPType != null && allVIPType.size() > 0) {
		...
		Map.Entry[] remnantVipTypeEntry = (Map.Entry[]) remnantSet.toArray(new Map.Entry[remnantSet.size()]);
	...
	try {
		...
		result.add(AjaxUtils.getAJAXPageData(httpRequest, GRID_FIELD_NAMES, GRID_FIELD_DEFAULT_VALUES, imPromotionItems, 
    ...
    public Map updateAJAXPromotion(Map parameterMap) throws FormException, Exception {
        ...
        try{
        	...
        	AjaxUtils.copyJSONBeantoPojoBean(formBindBean, promotionPO);       	    
    	        ...
    	        imPromotionItemService.checkPromotionData(promotionPO, PROGRAM_ID, identification);  //暫時註解Steve
...
public static Object[] startProcess(ImPromotion form) throws ProcessFailedException{       
        ...
        try{           
           ...
           String version = "20140515new";
           String sourceReferenceType = "ImPromotion (T1)";
           if("T2".equals(form.getBrandCode())){
           //version = "20091231T2";
           version = "20120710T2";
           sourceReferenceType = "ImPromotion (T2)";
           }
    ...
    public void updatePromotionStatus(Long headId, String status) throws Exception {
	    ...
	    if(promotion != null){
	        ...
	        if("T2".equalsIgnoreCase(promotion.getBrandCode()) ){
	            // 設定促銷活動代碼
	            String promotionCode = imItemCategoryService.getOrderSerialNo(promotion.getBrandCode(), ImItemCategoryDAO.ITEM_CATEGORY, promotion.getItemCategory());
    ...
    public List<Properties> getAJAXSearchPageData(Properties httpRequest) throws Exception{
    	...
    	try{
    	    ...
    	    String category17 = httpRequest.getProperty("category17");
    	    ...
    	    findObjs.put("category17", category17);
...
//				MailUtils.sendMail(subject, templateFileName, display, mailAddressList, attachFiles, cidMap);
    ...
    public List<Properties> getAJAXPageDataForReCombine(Properties httpRequest) throws Exception {
    	log.info("8888");
    	try {
    	    List<Properties> result = new ArrayList();
    	    List<Properties> gridDatas = new ArrayList();
    	    ...
    	    map.put("formStatus", formStatus);
    	    // ======================取得頁面所需資料===========================
    	    List<ImPromotionReCombine> imPromotionReCombines = imPromotionReCombineDAO.findPageLine(headId, iSPage, iPSize);
    		...
    		// 取得最後一筆 INDEX
    		Long maxIndex = imPromotionReCombineDAO.findPageLineMaxIndex(headId);
    		//refreshShopData(map, imPromotionReCombines);
    		result.add(AjaxUtils.getAJAXPageData(httpRequest,GRID_FIELD_NAMES_RECOMBINE, GRID_FIELD_DEFAULT_VALUES_RECOMBINE,
    				imPromotionReCombines, gridDatas, firstIndex, maxIndex));
    		log.info("55555");
    	    } else {
    	    	log.info("ssss");
    		result.add(AjaxUtils.getAJAXPageDataDefault(httpRequest,GRID_FIELD_NAMES_RECOMBINE, GRID_FIELD_DEFAULT_VALUES_RECOMBINE, gridDatas));
    	    }
