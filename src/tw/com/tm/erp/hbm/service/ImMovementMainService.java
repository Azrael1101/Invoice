...
public class ImMovementMainService {
	public static final String PROGRAM_ID = "IM_MOVEMENT";
	public static final String SCAN_CODE = "_SCAN_CODE";
	private static final Log log = LogFactory.getLog(ImMovementMainService.class);
	...
	public static final String[] GRID_FIELD_NAMES = { "indexNo", "boxNo", "itemCode", "reserve5", "itemName",
			...
			"originalDeclarationDate", "lineId", "isLockRecord", "isDeleteRecord", "returnMessage" ,"unit"};
	...
	public void resetLineReserve(List<ImMovementItem> resetObj) {
		for (ImMovementItem item : resetObj) {
			...
			item.setReserve4("");
			item.setReserve5("");
			log.info("Henry watch resetLineReserve==="+item.getDeliveryQuantity()+" headId"+resetObj.get(0).getImMovementHead().getHeadId());
		...
		for (ImMovementItem imMovementItem : imMovementItems) {
			...
			if (StringUtils.hasText(imMovementItem.getItemCode())) {
				...
				if ("Okay".equals(imMovementItem.getReserve1().trim())) {
					...
					log.info("Henry watch 1==="+imMovementItem.getDeliveryQuantity()+" headId:"+imMovementItem.getImMovementHead().getHeadId());
				} else {
	...
	*/
	...
	public void reflashImMovementItem(Map checkInfo, ImMovementItem itemObj) throws FormException, NoSuchDataException {
		...
		String allFixSuppliers = (String) checkInfo.get("allFixSuppliers");
		...
		if (imItem != null) {
			if (AjaxUtils.IS_DELETE_RECORD_TRUE.equals(itemObj.getIsDeleteRecord())) {
				...
				itemObj.setItemName(imItem.getItemCName());
				itemObj.setGoodDesc(imItem.getItemCName()==null?imItem.getItemCName():imItem.getItemCode());//goodDesc
				itemObj.setUnit(imItem.getPurchaseUnit()==null?"UNIT":imItem.getPurchaseUnit());//unit waitwait
				itemObj.setSpecDesc(imItem.getItemCName());//SPEC 
				log.info("商品單位以及說明:"+imItem.getPurchaseUnit()+" "+imItem.getItemCName());
				...
				if (TAX_MODE_FREE.equalsIgnoreCase(taxType)
						&& (!customsDeliveryWarehouseCode.equalsIgnoreCase(customsArrivalWarehouseCode)
								||orderTypeCode.equals("WMF"))) //by jason 20160325  檢查WMF報單
				{
				...
				} else if(!orderTypeCode.equals("WMF")){
				...
				if (OrderStatus.FINISH.equals(beforeStatus) || OrderStatus.CLOSE.equals(beforeStatus)
					...
					if (OrderStatus.WAIT_OUT.equals(beforeStatus)) {
						...
						if (null == itemObj.getArrivalQuantity())
							log.info("Henry watch this method reflashImMovementItem:"+itemObj.getDeliveryQuantity()+" headId:"+itemObj.getImMovementHead().getHeadId());
...
/*
	public HashMap getStatus(String formActoin, Long formId, Long processId, String formStatus, String approvalResult,
			String orderTypeCode, String orderNo, String brandCode,HashMap statusMap) {
		...
		// 判斷為兩點或三點調撥
		 ...
		 log.info("babu1000:"+isSigning);
	//	BuOrderTypeService service = (BuOrderTypeService) SpringUtils.getApplicationContext().getBean("buOrderTypeService");  
	//	boolean isProcessing = service.findOrderTypeApprovalById(brandCode +orderTypeCode+"001") == null? false:true;
		if (FORM_SUBMIT.equalsIgnoreCase(formActoin) || FORM_SUBMIT_BG.equalsIgnoreCase(formActoin)) {
			if (formId == null || AjaxUtils.isTmpOrderNo(orderNo)) {
				...
				// status = isThreePointMoving ? OrderStatus.WAIT_OUT :
					  ...
					  statusMap.put("formStaus", "SIGNING");
				...
				if (formStatus.equals(OrderStatus.SAVE) || formStatus.equals(OrderStatus.UNCONFIRMED)
					 ...
					 if(!isSigning){
						 ...
						 statusMap.put("formStaus", "SIGNING");
						 statusMap.put("afterStatus", "FINISH");
				 }else{
					 // status = isThreePointMoving ? OrderStatus.WAIT_OUT ;
					// OrderStatus.WAIT_IN;
					...
					status = OrderStatus.FINISH;
					statusMap.put("formStaus", "FINISH");
				} else if (formStatus.equals(OrderStatus.WAIT_OUT)) {
					status = OrderStatus.WAIT_IN;
					statusMap.put("formStaus", "WAIT_IN");
				}
			}
			...
			if (formStatus.equals(OrderStatus.SAVE) || formStatus.equals(OrderStatus.UNCONFIRMED)
				...
				statusMap.put("formStaus", "WAIT_OUT");
			}

		} else if (FORM_VOID.equalsIgnoreCase(formActoin)) {
			status = OrderStatus.VOID;
			statusMap.put("formStaus", "VOID");
		}
...
*/
	...
	public String getStatus(String formActoin, Long formId, Long processId, String formStatus, String approvalResult,
		...
		if (FORM_SUBMIT.equalsIgnoreCase(formActoin) || FORM_SUBMIT_BG.equalsIgnoreCase(formActoin)) {//假如單據送出或背景送出 會跑下面判斷式  註：equalsIgnoreCase不考慮大小寫
			if (formId == null || AjaxUtils.isTmpOrderNo(orderNo)) {//判斷formId為null值或是前端是暫存單號的話 會跑下面判斷式
				if(formStatus.equals(OrderStatus.UNCONFIRMED)){//假如單據送出 狀態為未確認時
				   status = OrderStatus.WAIT_IN;//狀態則為代轉入   為兩點式調撥
				}else{
				   status = OrderStatus.SIGNING;//否則為簽核中
				}
				   // else
				// status = isThreePointMoving ? OrderStatus.WAIT_OUT :
				// OrderStatus.WAIT_IN;
			} else if (processId != null || formStatus.equals(OrderStatus.UNCONFIRMED)) {//假如有進流程或是送出狀態為未確認時 跑下面判斷式
				if (formStatus.equals(OrderStatus.SAVE) || formStatus.equals(OrderStatus.UNCONFIRMED)//假如狀態為暫存或是狀態是未確認或是駁回 重新送出(submit)時
						|| formStatus.equals(OrderStatus.REJECT)) {
					// if(isSigning)
					log.info("暫存 未確認 駁回狀態下 重新送出(submit)時 狀態為WAIT_IN");
					status = OrderStatus.WAIT_IN;//狀態為WAIT_IN
					// else
					// status = isThreePointMoving ? OrderStatus.WAIT_OUT :
					// OrderStatus.WAIT_IN;
	...
	public void updateMoveUncommitQuantity(ImMovementHead imMovementHead, boolean isAllowMinus) throws FormException,
		...
		for (ImMovementItem imMovementItem : imMovementItems) {
			...
			if (quantity != 0) {
				if (isAllowMinus) {
					imOnHandDAO.updateMoveUncommitQuantityAllowMinus(organizationCode, imMovementItem.getItemCode(),
	...
	public static Object[] startProcess(ImMovementHead form) throws Exception {
		...
		try {
			...
			String version = "4";
			String sourceReferenceType = "ImMovement (4)";
	 ...
	 * @throws Exception
	 */
	public static Object[] startProcess01(ImMovementHead form) throws Exception {
		log.info("startProcess百貨新流程");
		try {
			String packageId = "Im_Movement";
			String processId = "approval";
			// String version = "T2".equals(form.getBrandCode())?"3":"2";
			...
			String version = "5";
			String sourceReferenceType = "ImMovement (5)";
	...
	public void insertMovement(ImMovementHead saveObj) throws ObtainSerialNoFailedException {
		...
		if ("unknow".equals(serialNo)) {
			...
			saveObj.setOrderNo(serialNo);
			resetLineReserve(saveObj.getImMovementItems());
			//ImWarehouse w3 = imWarehouseDAO.findById(saveObj.getDeliveryWarehouseCode());
            //ImWarehouse imDeliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class, w3.getWarehouseId());
            ...
            //ImWarehouse w4 = imWarehouseDAO.findById(saveObj.getArrivalWarehouseCode());
            //ImWarehouse imArrivalWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class, w4.getWarehouseId());
	...
	private void refreshImMovement(HashMap parameterMap, List<ImMovementItem> imMovementItems) throws Exception {
		...
		//ImWarehouse imDeliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class,deliveryWarehouseCode);
		//ImWarehouse w1 = (ImWarehouse) imWarehouseDAO.findById(deliveryWarehouseCode);
	...
	public void saveTmp(ImMovementHead imMovementHead) throws Exception {
		...
		try {
			...
			//imReceiveHeadDAO.save(imMovementHead);
		} catch (Exception ex) {
	...
	public List<Properties> updateAJAXPageLinesData(Properties httpRequest) throws Exception {
		...
		try {
			...
			String employeeCode = httpRequest.getProperty("employeeCode");//20160425 by jason
			...
			String status = imh.getStatus();
			String errorMsg = null;
			
			
			log.info("updateAJAXPageLinesData.status:" + status + "headId : "+headId);
			if (OrderStatus.SAVE.equals(status) || OrderStatus.REJECT.equals(status)
				...
				if (upRecords != null) {
					for (Properties upRecord : upRecords) {
						...
						log.info("updateAJAXPageLinesData.lineId:" + lineId + "headId : "+headId);
						...
						if (StringUtils.hasText(itemCode)) {
							log.info("check imMovementItem headId :" + headId +" lineId : "+lineId);
							...
							if (imMovementItemUp != null) {
								...
								//imMovementItemUp.setArrivalQuantity(imMovementItemUp.getDeliveryQuantity());
								if(OrderStatus.SAVE.equals(status)){ 
									Map desc = getDescValue(brandCode, orderTypeCode, itemCode);
									imMovementItemUp.setGoodDesc((String)desc.get("goodDesc"));
									imMovementItemUp.setSpecDesc((String)desc.get("specDesc"));
								...
								if(imMovementItemUp.getItemCode()!=null||imMovementItemUp.getItemName()!=null){
									...
									ims.setAction("updateAJAXPageLinesData");
									...
									ims.setMessage(imMovementItemUp.getItemCode()+" "+imMovementItemUp.getArrivalQuantity().toString());
									ims.setModule_name("Im_Movement");
									imSysLogDAO.merge(ims);
									ims=null;
								...
								//20160425 by jason  imMovementItem 建立與最後修改人員與日期回寫
								if(imMovementItem.getCreatedBy() == null ||imMovementItem.getCreatedBy().length() == 0){
									imMovementItem.setCreatedBy(employeeCode);
									imMovementItem.setLastUpdatedBy(employeeCode);
								}else{
						 			imMovementItem.setLastUpdatedBy(employeeCode);
								}
								if(imMovementItem.getCreationDate() == null ){
								...
								//by jason  修正維修單連續建單gooddesc、specdesc無法寫入
								Map desc = getDescValue(brandCode, orderTypeCode, itemCode);
								imMovementItem.setGoodDesc((String)desc.get("goodDesc"));
								imMovementItem.setSpecDesc((String)desc.get("specDesc"));
								////////**********////////
	...
	public Map getDescValue(String brandCode, String orderTypeCode, String itemCode) throws Exception{
		Map result = new HashMap();
		try{
			if("WMF".equals(orderTypeCode)){
				ImItem imItem = imItemDAO.findItem(brandCode, itemCode);
				if(imItem == null) {
					log.info("111ddd----imItem=null");
					throw new Exception("品項錯誤,找不到此商品" );
				}else{
					log.info("imItem.getItemCode():"+imItem.getItemCode()+" imItem.getItemCName():"+imItem.getItemCName());
					result.put("goodDesc", imItem.getItemCName()==null
								?imItem.getItemCName():imItem.getItemCode());
					result.put("specDesc", imItem.getItemCName());									
				}
			}
		}catch(Exception ex){
			ex.printStackTrace();
	...
	public List<Properties> executeInitial(Map parameterMap) throws Exception {
		...
		try {
			...
			//依照品號重新排序indexNo
			this.removeDetailItemCode(form);
			this.checkArrivalWarehouse(form);	
			...
			//log.info("1ddddd-----allArrivalWarehouses:   ");
			...
			List<ImWarehouse> allArrivalWarehouses = this.getWarehouseInfo("arrival", form.getOrderTypeCode(), form.getBrandCode(), employeeCode,//by jason
			 itemCategoryMode);
			List<ImWarehouse> allArrivalStore = null;
			
			//維修單廠商
			List allFixSuppliers = buSupplierWithAddressViewDAO.findRp();
			log.info("維修廠商數量:"+allFixSuppliers.size());
			...
			multiList.put("customsStatusCode", form.getCustomsStatus());
			this.setMoveTypeArray(allOrderTypes);
			
			
			multiList.put("allFixSuppliers", AjaxUtils.produceSelectorData(allFixSuppliers, "supplierCode",
					"chineseName", true, true));
	...
	public ImMovementHead executeNewImMovement(Object otherBean, Map resultMap) throws Exception {
		...
		List<ImWarehouse> allArrivalWarehouses = this.getWarehouseInfo("arrival", orderTypeCode, loginBrandCode, loginEmployeeCode,//by jason
				itemCategoryMode);
	...
	public ImMovementHead findImMovement(Object otherBean, Map resultMap) throws Exception {
		...
		if (null != form) {
			...
			System.out.println("CUSTOMS_STATUS:"+form.getCustomsStatus());

			ImWarehouse imDeliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class, form.getDeliveryWarehouseCode());
			//ImWarehouse w2 = imWarehouseDAO.findById(form.getDeliveryWarehouseCode());
            //ImWarehouse imDeliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class, w2.getWarehouseId());
			...
			if (!(OrderStatus.SAVE.equals(form.getStatus()) && OrderStatus.UNCONFIRMED.equals(form.getStatus()))) {
				...
				ImWarehouse imArrivalWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class, form.getArrivalWarehouseCode());
			//	ImWarehouse w3 = imWarehouseDAO.findById(form.getArrivalWarehouseCode());
              //  ImWarehouse imArrivalWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class, w3.getWarehouseId());
			...
			//修正調撥單初始化失敗錯誤
			List<ImMovementItem> imMovementItems = form.getImMovementItems();
			for(ImMovementItem imMovementItem:imMovementItems){
				if(imMovementItem != null){
					if (imMovementItem.getOriginalDeliveryQuantity() != null)
						totalOriginalDeliveryQuantity = totalOriginalDeliveryQuantity
								+ imMovementItem.getOriginalDeliveryQuantity();
					if (imMovementItem.getDeliveryQuantity() != null)
	...
	public List<ImWarehouse> getWarehouseInfo(String type, String orderTypeCode, String brandCode, String employeeCode,
		...
		//111ddd
		if(this.isWarehouseExecute()){
		return imWarehouseDAO.findMovementWarehouse(type, findObjs);
		}else{
			return imWarehouseDAO.findMovementWarehouseByEmployee(type, findObjs);
		}
...
*/
	...
	public HashMap updateAJAXMovement(Map parameterMap) throws FormException, Exception {
		...
		try {
			...
			Object formLinkBean = parameterMap.get("vatBeanFormLink");
			Object otherBean = parameterMap.get("vatBeanOther");
			log.info("otherBean.toString():"+otherBean.toString());
			//原版
			ImMovementHead MovementPO = (ImMovementHead)parameterMap.get("imMovementHead");
			//原版
		//	Long headId = (Long) PropertyUtils.getProperty(otherBean, "headId");
			
			//ImMovementHead MovementPO = findById(Long.parseLong("3958097"));
			log.info("確認單頭資料是否儲存:"+MovementPO.getArrivalStoreCode());
			imMovementHeadDAO.update(MovementPO);  //儲存單頭
			...
			BuOrderTypeService service = (BuOrderTypeService) SpringUtils.getApplicationContext().getBean("buOrderTypeService");  
			boolean isProcessing = service.findOrderTypeApprovalById(MovementPO.getBrandCode() +MovementPO.getOrderTypeCode()+"001") == null? false:true;
			
			
			
			log.info("modHead.getItemCount2():"+MovementPO.getItemCount());
			
		
			this.setOriginalOrder(MovementPO, identification, errorMsgs);    //22222222222222222222222222222222222222222222222222222222222
			
			log.info("modHead.getItemCount3():"+MovementPO.getItemCount());
	log.info("pppppppppppppppooooo:"+parameterMap.get("beforeStatus").toString()+"     "+parameterMap.get("afterStatus").toString());		
			String beforeStatus = parameterMap.get("beforeStatus").toString();
			String afterStatus = parameterMap.get("afterStatus").toString();
			MovementPO.setStatus(afterStatus);
			log.info("bbbbbbbbbeforeStatus:"+beforeStatus);
			log.info("aaaaaafterStatus:"+afterStatus);
			log.info("表單Status:"+MovementPO.getStatus());
			log.info("表單動作:"+formAction);
			
			boolean isThreePointMoving = this.isThreePointMoving(MovementPO.getBrandCode(), MovementPO
					.getOrderTypeCode());
			log.info("modHead.getItemCount4():"+MovementPO.getItemCount());
			log.info("是否執行流程:"+isProcessing);
			if (OrderStatus.FORM_SUBMIT.equals(formAction)){
			if(isProcessing==true){
				log.info("送出..跑流程");
				 if(OrderStatus.SAVE.equalsIgnoreCase(beforeStatus)){
				         if(isThreePointMoving){
				        	 log.info("送出..跑流程..三點調撥");
			                MovementPO.setStatus("WAIT_OUT");
			               }else{
			            	   log.info("送出..跑流程..非三點調撥");
					        MovementPO.setStatus("WAIT_IN");	
				           }
				  }
			  }else{
				  log.info("送出..不跑流程的單據");
			...
			}else if(OrderStatus.SAVE.equals(formAction)){
				 log.info("暫存");
				MovementPO.setStatus("SAVE");	
			} 
			imMovementHeadDAO.update(MovementPO);  //儲存單頭
		
			
			log.info("saveAjaxData_1:"+MovementPO.getItemCount());
			String resultMsg = this.saveAjaxData(MovementPO, formAction, identification, errorMsgs, beforeStatus,afterStatus,isProcessing);
	...
	public HashMap updateAJAXMovementParcial(Map parameterMap) throws FormException, Exception {
		log.info("進入前端資料塞入Bean時間點"+new Date());
		
		System.out.println("4.updateAJAXMovement:取前端資料塞入Bean開始:更新單頭以及最後更新日期以及人員");
		MessageBox msgBox = new MessageBox();
		HashMap resultMap = new HashMap(0);
		...
		try {
			...
			// 取得欲更新的bean
			ImMovementHead MovementPO = getActualMovement(formLinkBean);
			 List<ImMovementItem> items = MovementPO.getImMovementItems();
			for(ImMovementItem item:items)
			{
				item.setArrivalQuantity(item.getDeliveryQuantity());
				//item.setArrivalWarehouseCode(MovementPO.getArrivalWarehouseCode());
				//item.setIndexNo(item.getIndexNo());
				log.info("setIndexNo==="+item.getIndexNo());
				
				  ImSysLog ims = new ImSysLog();
	                ims.setHeadId(MovementPO.getHeadId());
	                ims.setAction("updateAJAXMovementParcial");
	                ims.setCreate_date(new Date());
	                ims.setCreated_by(MovementPO.getCreatedBy());
	                ims.setIdentification(MovementPO.getBrandCode()+MovementPO.getOrderTypeCode()+MovementPO.getOrderNo());
	                ims.setMessage(item.getItemCode()+" DeliveryQuantity: "+item.getDeliveryQuantity()+" ArrivalQuantity: "+item.getArrivalQuantity());
	                ims.setModule_name("Im_Movement");
	                imSysLogDAO.merge(ims);
	                ims=null;

			}
			MovementPO.setImMovementItems(items);
			
			
		//	AjaxUtils.copyJSONBeantoPojoBean(formBindBean, MovementPO);
			String employeeCode = (String) PropertyUtils.getProperty(otherBean, "loginEmployeeCode");//前端傳來的登入工號
			String rpSupplier = (String) PropertyUtils.getProperty(otherBean, "fixSuppliers");//前端傳來的登入工號
			MovementPO.setRpSupplier(rpSupplier);
			MovementPO.setLastUpdatedBy(employeeCode);//更新異動人員
			MovementPO.setLastUpdatedBy(employeeCode);//更新異動人員
			MovementPO.setLastUpdateDate(new Date());//更新異動日期
			System.out.println("4.updateAJAXMovement:取前端資料塞入Bean結束");
			System.out.println("儲存單頭開始 updateAJAXMovementParcial");
			imMovementHeadDAO.update(MovementPO);//update更新的資料
			System.out.println("更新單頭結束 updateAJAXMovementParcial");
			resultMap.put("entityBean", MovementPO);
	...
	*/
     ...
     */
    public boolean removeDetailItemCode(ImMovementHead headObj) {
    	log.info("進入removeDetailItemCode時間點"+new Date());
    	System.out.println("移除空白的 imReceiveItem : checkDetailItemCode單別:"+headObj.getOrderTypeCode()+" 單號:"+headObj.getOrderNo());
		List<ImMovementItem> items = imMovementHeadDAO.getItems(headObj.getHeadId());
		List<ImMovementItem> isDelItems = new ArrayList();
		...
		headObj.setImMovementItems(items);
		return items.isEmpty();
	}
    
    public ImMovementHead checkArrivalWarehouse(ImMovementHead headObj){
    	String arrivalWarehouse = headObj.getArrivalWarehouseCode();
    	List<ImMovementItem> imMovementItems = headObj.getImMovementItems();
    	for(ImMovementItem imMovementItem:imMovementItems){
    		if(!arrivalWarehouse.equals(imMovementItem.getArrivalWarehouseCode())){
    			imMovementItem.setArrivalWarehouseCode(arrivalWarehouse);
    		}
    	}
    	return headObj;
    }
	...
	public String saveAjaxData(ImMovementHead modifyObj, String formAction, String identification, List errorMsgs,
			String beforeStatus,String afterStatus,boolean isProcessing) throws Exception {
		log.info("5.3 saveAjaxDatadddddddd:"+modifyObj.getItemCount());
		/*移倉變數宣告 Start*/
		List lst_STORET1;
		List lst_STORET2;
		String RefBillNo = "";
				...
				if (OrderStatus.FORM_SUBMIT.equals(formAction) || FORM_SUBMIT_BG.equals(formAction)) {
					...
					if (errorMsgs.size() == 0) {
						...
						if ((OrderStatus.SIGNING.equalsIgnoreCase(modifyObj.getStatus()) && (OrderStatus.UNCONFIRMED.equalsIgnoreCase(beforeStatus)
								...
								|| (OrderStatus.WAIT_OUT.equalsIgnoreCase(modifyObj.getStatus()))//百貨用
					  ...
					  if(!isProcessing){
						  log.info("是否流程?"+afterStatus+" "+beforeStatus);
						  if((OrderStatus.SAVE.equalsIgnoreCase(beforeStatus)&&OrderStatus.WAIT_IN.equalsIgnoreCase(afterStatus))){	
							  log.info("不跑流程");
							  modifyObj.setStatus("FINISH");
							  updateMoveOnHand(modifyObj, false, identification, errorMsgs, 
										StringUtils.hasText(orderTypeSetting.getAttribute5()) ? orderTypeSetting.getAttribute5() : "NORMAL", false);
							//for 儲位用
					    		if(imStorageAction.isStorageExecute(modifyObj)){
						...
						log.info("modify.getItemCount:"+modifyObj.getItemCount());
						update(modifyObj);
						returnResult = modifyObj.getOrderTypeCode() + "-" + modifyObj.getOrderNo() + "存檔成功";
						log.info("回傳文字訊息:"+returnResult);
					}
	 ...
	 * @throws Exception
	 */
	public void executeImportMovementItems(Long headId, String deliveryWarehouseCode, String arrivalWarehouseCode, List movementItems) throws Exception {
		log.info("executeImportMovementItems.." + headId + "--" + deliveryWarehouseCode + "--" + arrivalWarehouseCode);
		log.info("executeImportMovementItems..size" + movementItems.size());
		List<ImMovementItem> items = new ArrayList(0);
		try {
			// deleteMovementItems(headId);
			...
			if (movementItems != null && movementItems.size() > 0) {
				for (int i = 0; i < movementItems.size(); i++) {
					ImMovementItem imMovementItem = (ImMovementItem) movementItems.get(i);
					imMovementItem.setDeliveryWarehouseCode(deliveryWarehouseCode);//KMF-Jerome
					imMovementItem.setArrivalWarehouseCode(arrivalWarehouseCode);//KMF-Jerome
					imMovementItem.setDeliveryQuantity(imMovementItem.getDeliveryQuantity()* -1);//KMF-Jerome
	...
	public List updateCheckedImMovementData(Map parameterMap) throws Exception {
		...
		try {
			...
			//checkImMovement(MovementPO, identification, errorMsgs, orderTypeSetting, MovementPO.getStatus());    待修改checkImMovement參數
	...
	public ImMovementHead checkImMovement(Map parameterMap) throws Exception {
		log.info("進入檢核單頭以及單身資料正確性時間點"+new Date());
		...
		Object formLinkBean = parameterMap.get("vatBeanFormLink");
		Object otherBean = parameterMap.get("vatBeanOther");
		String allFixSuppliers = (String) PropertyUtils.getProperty(formBindBean, "rpSupplier");
		String employeeCode = (String) PropertyUtils.getProperty(otherBean, "loginEmployeeCode");//前端傳來的值登入工號
		String formAction = (String) PropertyUtils.getProperty(otherBean, "formAction");//狀態參數
		String orderTypeCode = (String) PropertyUtils.getProperty(formBindBean, "orderTypeCode");
		Long processId = NumberUtils.getLong((String) PropertyUtils.getProperty(otherBean, "processId"));//流程Id
		// 取得欲更新的bean
		
		HashMap resultMap = (HashMap)parameterMap.get("resultMap");
		ImMovementHead checkObj = (ImMovementHead)resultMap.get("entityBean");
		
		//ImMovementHead checkObj = getActualMovement(formLinkBean);
		
		log.info("5.3.1 checkImMovement...Test");
		String beforeStatus = checkObj.getStatus();//取出STATUS
		log.info("5.3.1 checkImMovement...Test1");
		//statusMap.put("beforeStatus", beforeStatus);
		System.out.println("beforeStatus=" + beforeStatus);
		
		String formStatus = getStatus(formAction, checkObj.getHeadId(), processId, checkObj.getStatus(), "",
				checkObj.getOrderTypeCode(), checkObj.getOrderNo());//取得狀態給formStatus變數(傳formAction：表單狀態；checkObj.getHeadId()：pk；processId：流程用id；checkObj.getStatus()：取得狀態)
		log.info("5.3.1 checkImMovement...Test2:"+formStatus);//取得單別及單號等參數
        checkObj.setStatus(formStatus);
        log.info("5.3.1 checkImMovement...Test3:"+formStatus);
        checkObj.setLastUpdatedBy(employeeCode);
        log.info("5.3.1 checkImMovement...Test4:"+formStatus);
       log.info("之前Ststus:"+beforeStatus);
       log.info("之後Ststus:"+formStatus);
        
        parameterMap.put("beforeStatus", beforeStatus);
        parameterMap.put("afterStatus", formStatus);
        parameterMap.put("statusMap", statusMap);
        
        
        log.info("確認單頭資料是否儲存:"+checkObj.getDeliveryWarehouseCode());
        AjaxUtils.copyJSONBeantoPojoBean(formBindBean, checkObj); //將前端資料塞入Bean
        if (OrderStatus.FORM_SUBMIT.equals(formAction)){          //送出時才檢核單據
		if (null != checkObj) {
			BuCommonPhraseLine orderTypeSetting = buCommonPhraseLineDAO.findById("ImMovementOrderType", 
					checkObj.getBrandCode() + checkObj.getOrderTypeCode());
			log.info("5.3.1 checkImMovement...Test3");
		String identification = MessageStatus.getIdentification(checkObj.getBrandCode(),
				checkObj.getOrderTypeCode(), checkObj.getOrderNo());
		log.info("5.3.1 checkImMovement...Test4");
		siProgramLogAction.deleteProgramLog("IM_MOVEMENT", null, identification);
		...
		Boolean disallowMinusStock = new Boolean(true);
		String arrivalWarehouseCode = checkObj.getArrivalWarehouseCode();
		String deliveryWarehouseCode = checkObj.getDeliveryWarehouseCode();
		try {
			List<ImMovementItem> imMovementItems = checkObj.getImMovementItems();
			...
			ValidateUtil.isAfterClose(checkObj.getBrandCode(), checkObj.getOrderTypeCode(), "轉出日期", checkObj
					.getDeliveryDate(),checkObj.getSchedule());
			if (OrderStatus.FINISH.equals(checkObj.getStatus())) {
			...
			// checkObj.getStatus())
				...
				//ImWarehouse deliveryWarehouse = (ImWarehouse) imWarehouseDAO.findById(checkObj.getDeliveryWarehouseCode());//.findById("ImWarehouse", Long.valueOf(checkObj
						//.getDeliveryWarehouseCode()));
				ImWarehouse deliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByBrandCodeAndWarehouseCode(checkObj.getBrandCode(), checkObj.getDeliveryWarehouseCode(), "Y");
				if (null == deliveryWarehouse)
					this.messageHandle(MessageStatus.LOG_ERROR, identification, "轉出倉別代號("
			...
			checkInfo.put("waitingWarehouseCode", waitingWarehouseCode);
			checkInfo.put("disallowMinusStock", disallowMinusStock);
			checkInfo.put("allFixSuppliers", allFixSuppliers);
			for (ImMovementItem imMovementItem : imMovementItems) {
				i++;
				if (StringUtils.hasText(imMovementItem.getItemCode())
					...
					// call new reflashImMovementItem
					if(!imMovementItem.getArrivalWarehouseCode().equals(arrivalWarehouseCode)){
						imMovementItem.setArrivalWarehouseCode(arrivalWarehouseCode);
					}
					if(!imMovementItem.getDeliveryWarehouseCode().equals(deliveryWarehouseCode)){
						imMovementItem.setDeliveryWarehouseCode(deliveryWarehouseCode);
					}

					this.reflashImMovementItem(checkInfo, imMovementItem);
					log.info("Henry watch this method checkImMovement:"+imMovementItem.getArrivalQuantity()+" headId:"+imMovementItem.getImMovementHead().getHeadId());
					imMovementItem.setArrivalQuantity(imMovementItem.getDeliveryQuantity());
			...
			log.info("before remove size:" + imMovementItems.size());
			//by jason 20160325 檢核WMF是否有選擇維修廠商

			if(orderTypeCode.equals("WMF")&&allFixSuppliers.length()==0){
				throw new Exception ("請選擇維修廠商");
			}
		...
		log.info("錯誤訊息數量 =====>"+errorMsgs.size());
		 if(errorMsgs.size() > 0){
			throw new ValidationErrorException(MessageStatus.VALIDATION_FAILURE);
		 }
		}
}
        removeDetailItemCode(checkObj);  //待確認--刪除修正
		...
		// }
	    //imMovementHeadDAO.update(checkObj);
		
		 return checkObj;
	}
    
	public void checkImMovementOri(ImMovementHead checkObj, String identification, List errorMsgs,
		...
		try {
			...
			// checkObj.getStatus())
				...
				//ImWarehouse deliveryWarehouse = (ImWarehouse) imWarehouseDAO.findById("ImWarehouse", checkObj.getDeliveryWarehouseCode());
				ImWarehouse deliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByBrandCodeAndWarehouseCode(checkObj.getBrandCode(), checkObj.getDeliveryWarehouseCode(), "Y");
				if (null == deliveryWarehouse)
					this.messageHandle(MessageStatus.LOG_ERROR, identification, "轉出倉別代號("
							+ checkObj.getDeliveryWarehouseCode() + ")錯誤", user, errorMsgs);
	...
	public ImMovementHead saveActualOrderNo(Long headId, String loginUser) throws ObtainSerialNoFailedException,
		...
		System.out.println("取調撥單正式單號開始");
		log.info("進入取調撥單正式單號時間點"+new Date());
		ImMovementHead imMovementPO = this.findById(headId);//用headId找暫存單號
		if (imMovementPO == null) {//假如物件無直跳Exception
			throw new NoSuchObjectException("查無調撥單主鍵：" + headId + "的資料！");
		} else { // 取得正式的單號
			...
			if(imStorageAction.isStorageExecute(imMovementPO)){
				...
				this.setOrderNo(imMovementPO);//取得正式單號
			}
			
			
		}
		imMovementPO.setLastUpdatedBy(loginUser);//更新異動人員
		imMovementPO.setLastUpdateDate(new Date());//更新異動日期
		System.out.println("取調撥單正式單號結束");
		System.out.println("更新調撥單開始:更新單號以及最後更新日期及人員開始");
		imMovementHeadDAO.update(imMovementPO);//更新成正式單號
		System.out.println("更新調撥單開始:更新單號以及最後更新日期及人員結束");
		
		return imMovementPO;//回傳物件
	}
	
	/*public void saveRpNo(Long headId, String loginUser) throws NoSuchObjectException{
		System.out.println("取維修申請書號碼開始");
		
		ImMovementHead imMovementPO = this.findById(headId);//用headId找暫存單號
		if (imMovementPO == null) {//假如物件無直跳Exception
			throw new NoSuchObjectException("查無調撥單主鍵：" + headId + "的資料！");
		} else {
			Long seq = Long.valueOf(imMovementHeadDAO.getWhNoSq().get(0).toString()) + 1 ;
			Calendar cl = Calendar.getInstance();
			cl.setTimeInMillis(System.currentTimeMillis());
		    int year=cl.get(Calendar.YEAR)-1911;
			String RpNo = "TCG084" + String.valueOf(year).substring(1) + String.format("%06d",seq) ;
			//System.out.println("LLLLLLLLLLLLLLLLLLLLLLLL:"+RpNo);
			imMovementPO.setSendRpNo(RpNo);
		}
		imMovementPO.setLastUpdatedBy(loginUser);//更新異動人員
		imMovementPO.setLastUpdateDate(new Date());//更新異動日期
		//imMovementPO.setCustomsStatus("A01");
		System.out.println("取維修申請書號碼結束");
		System.out.println("更新維修申請書號碼開始:更新單號以及最後更新日期及人員開始");
		imMovementHeadDAO.update(imMovementPO);//更新成正式單號
		System.out.println("更新維修申請書號碼開始:更新單號以及最後更新日期及人員結束");
	}*/
	...
	public void updateMoveOnHand(ImMovementHead imMovementHead, boolean allowCmStockMinus, String identification,
		...
		try {
			...
			warehouseCode = OrderStatus.FINISH.equals(imMovementHead.getStatus()) ? imMovementHead.getArrivalWarehouseCode()  
					: imMovementHead.getDeliveryWarehouseCode();    //若單據為FINISH 則扣轉出庫的帳
			log.info("DDDDDora:"+imMovementHead.getStatus());
			//ImWarehouse w9 = imWarehouseDAO.findById(imMovementHead.getDeliveryWarehouseCode());
			ImWarehouse imDeliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class,
			...
			if (null == imDeliveryWarehouse) {
				...
				//ImWarehouse w10 = imWarehouseDAO.findById(imMovementHead.getArrivalWarehouseCode());
				...
				if (null == imArrivalWarehouse) {
					...
					Map imOnHandCompareMaps = new HashMap();
					Map cmOnHandCompareMaps = new HashMap();
					...
					for (ImMovementItem imMovementItem : imMovementItems) {
						...
						/*Double imAvaQty = 
							imOnHandDAO.getStockOnHandQty(organizationCode, imMovementItem.getItemCode() 
									, warehouseCode , imMovementItem.getLotNo(), brandCode);
							imOnHandCompareMaps.put(imMovementItem.getItemCode(), imAvaQty == null? 0:imAvaQty);*/
						
						
						if (diffCustomsWarehouse){
							this.collectCmOnHand(imMovementItem, cmOnHandMaps, i, cmWarehouseCode, quantity);
							String cmAvaId = imMovementItem.getOriginalDeclarationNo()+imMovementItem.getOriginalDeclarationSeq()+imMovementItem.getItemCode();
							Double cmAvaQty = 
								cmDeclarationOnHandDAO.getCurrentOnHandQtyByProperty(brandCode, imMovementItem.getOriginalDeclarationNo()
										, imMovementItem.getOriginalDeclarationSeq(), imMovementItem.getItemCode(), cmWarehouseCode);
							//cmOnHandCompareMaps.put( cmAvaId, cmAvaQty == null? 0:cmAvaQty);
						}
						i++;
					}
...
//					if (!imMovementHead.getOrderTypeCode().equals("KMF")){
						log.info("20170308-----");
					    upOnhand.updateOnhandData(imOnHandMaps, cmOnHandMaps, transactionType, imMovementHead.getBrandCode(), disallowMinusStock, allowCmStockMinus, organizationCode
							             , isClose, isRevert, imMovementHead.getLastUpdatedBy(),identification,errorMsgs, isServiceItemMap, allowWholeSale, soSalesOrderHead, aggregateResult);//新版扣庫存
//					}
					
					//新版
//					upOnhand.updateOnhandData2(imOnHandMaps, cmOnHandMaps, transactionType, imMovementHead.getBrandCode(), allowImStockMinus, allowCmStockMinus, organizationCode
//				             , isClose, isRevert, imMovementHead.getLastUpdatedBy(),identification,errorMsgs, imMovementHead, isServiceItemMap, allowWholeSale, soSalesOrderHead, aggregateResult);//新版扣庫存
					
					//＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝一致性檢查開始＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
					/*log.info("一致性檢查開始");
					List<ImMovementItem> imMovementCompareItems = imMovementHead.getImMovementItems();
					for(ImMovementItem imMovementItem:imMovementCompareItems){
						//目前可用庫存C
						Double imAvaQty = imOnHandDAO.getStockOnHandQty(organizationCode, imMovementItem.getItemCode() , warehouseCode , imMovementItem.getLotNo(),brandCode);
						//變動前庫存A
						Double imOnHandCompare = (Double)imOnHandCompareMaps.get(imMovementItem.getItemCode());
						if(imAvaQty == null){
							throw new Exception("organizationCode, imMovementItem.getItemCode() , warehouseCode , imMovementItem.getLotNo(),brandCode"+
									organizationCode+","+imMovementItem.getItemCode() +","+ warehouseCode +","+ 
									imMovementItem.getLotNo()+","+brandCode);
						}
						//＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝IM檢查＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
						if(imOnHandCompare.equals(imAvaQty)){
							throw new Exception("扣除庫存異常，原因：IM扣除前後一致");
						}else if(imOnHandCompare > imAvaQty){
							if(!imMovementItem.getDeliveryQuantity().equals((imOnHandCompare - imAvaQty))){
								if(imMovementItem.getDeliveryQuantity()< (imOnHandCompare - imAvaQty)){
									imOnHandCompareMaps.put(imMovementItem.getItemCode(), (imOnHandCompare - imAvaQty));
								}else{
									throw new Exception("扣除庫存異常，原因：扣除驗證錯誤");
								}
							}
						}else if(imOnHandCompare < imAvaQty){
							if(!imMovementItem.getDeliveryQuantity().equals((imAvaQty - imOnHandCompare))){
								if(imMovementItem.getDeliveryQuantity()< (imAvaQty - imOnHandCompare)){
									imOnHandCompareMaps.put(imMovementItem.getItemCode(), (imAvaQty - imOnHandCompare));
								}else{
									throw new Exception("扣除庫存異常，原因：扣除驗證錯誤");
								}
							}
						}
						log.info("IM ItemCode:"+imMovementItem.getItemCode()+",變動前庫存A:"+imOnHandCompare+",目前可用庫存C="+imAvaQty+",變動量:"+imMovementItem.getDeliveryQuantity()+"");
						//＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝CM檢查＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
						if (diffCustomsWarehouse){
							String cmAvaId = imMovementItem.getOriginalDeclarationNo()+imMovementItem.getOriginalDeclarationSeq()+imMovementItem.getItemCode();
							//目前可用庫存C
							Double cmAvaQty = 
								cmDeclarationOnHandDAO.getCurrentOnHandQtyByProperty(brandCode, imMovementItem.getOriginalDeclarationNo()
										, imMovementItem.getOriginalDeclarationSeq(), imMovementItem.getItemCode(), cmWarehouseCode);
							//變更前庫存A
							Double cmOnHandCompare = (Double)cmOnHandCompareMaps.get(cmAvaId);
							if(cmOnHandCompareMaps.get(cmAvaId).equals(cmAvaQty)){
								throw new Exception("扣除庫存異常，原因：CM扣除前後一致");
							}else if(cmOnHandCompare > cmAvaQty){
								if(!imMovementItem.getDeliveryQuantity().equals((cmOnHandCompare - cmAvaQty))){
									if(imMovementItem.getDeliveryQuantity()< (cmOnHandCompare - cmAvaQty)){
										imOnHandCompareMaps.put(imMovementItem.getItemCode(), (cmOnHandCompare - cmAvaQty));
									}else{
										throw new Exception("扣除庫存異常，原因：扣除驗證錯誤");
									}
								}
							}else if(cmOnHandCompare < cmAvaQty){
								if(!imMovementItem.getDeliveryQuantity().equals((cmAvaQty - cmOnHandCompare))){
									if(imMovementItem.getDeliveryQuantity()< (cmAvaQty - cmOnHandCompare)){
										imOnHandCompareMaps.put(imMovementItem.getItemCode(), (cmAvaQty - cmOnHandCompare));
									}else{
										throw new Exception("扣除庫存異常，原因：扣除驗證錯誤");
									}
								}
							}
							//log.info("CM ItemCode:"+imMovementItem.getItemCode()+"/balance=扣除前=現有+轉出量:"+cmBal+"="+cmOnHandCompareMaps.get(cmAvaId)+"="+cmAvaQty+"+"+imMovementItem.getDeliveryQuantity()+"檢查完畢");
						}
					}
					log.info("一致性檢查結束");*/
					//＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝一致性檢查結束＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	private String getWarehouseManager(String brandCode, String warehouseCode, String customsWarehouseCode,
		...
		if ("Y".equalsIgnoreCase(itemCategoryMode) && StringUtils.hasText(customsWarehouseCode)) {
			...
			//ImWarehouse w11 = (ImWarehouse) imWarehouseDAO.findById(warehouseCode);
		...
		if (movementItems != null && movementItems.size() > 0) {
			...
			for (int i = movementItems.size() - 1; i >= 0; i--) {
				...
				if (AjaxUtils.IS_DELETE_RECORD_TRUE.equals(movementItem.getIsDeleteRecord())) {
					...
					movementItems.remove(movementItem.getItemCode());
	...
	private void updateMoveOnHandtoVoid(ImMovementHead imMovementHead, boolean allowCmStockMinus, String identification,
		...
		this.updateMoveOnHandtoVoid(imMovementHead, allowCmStockMinus, identification, msgLists, onHandMode, "SAVE");
	}
	 ...
	 */
	private void updateMoveOnHandtoVoid(ImMovementHead imMovementHead, boolean allowCmStockMinus, String identification,
			List msgLists, String onHandMode, String toStatus) throws Exception {
		...
		try {
			...
			//ImWarehouse w3 = imWarehouseDAO.findById(imMovementHead.getDeliveryWarehouseCode());
         //   ImWarehouse imDeliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class, w3.getWarehouseId());
			...
			if (null == imDeliveryWarehouse) {
				...
				//ImWarehouse w4 = imWarehouseDAO.findById(imMovementHead.getArrivalWarehouseCode());
	           // ImWarehouse imArrivalWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class, w4.getWarehouseId());
				ImWarehouse imArrivalWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class,
						imMovementHead.getArrivalWarehouseCode());
				if (null == imArrivalWarehouse) {
		    		...
		    		if(imStorageAction.isStorageExecute(imMovementHead)){
		    			//異動儲位庫存，目前只能送出增加庫存，若為駁回isReject應為true
		    			imStorageService.updateStorageOnHandBySource(imMovementHead, OrderStatus.SAVE, null, null, true);
		    		}
				}
			}
		} catch (Exception ex) {
	...
	public List<Properties> changeItemCategory(Map parameterMap) throws Exception {
		...
		try {
			...
			ImWarehouse deliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByBrandCodeAndWarehouseCode(brandCode, deliveryWarehouseCode, "Y"); // 轉出庫
			//ImWarehouse deliveryWarehouse = (ImWarehouse) imWarehouseDAO.findById(deliveryWarehouseCode); // 轉出庫
			if (null != deliveryWarehouse) {
				log.info("111"+deliveryWarehouse.getWarehouseManager());
			...
			ImWarehouse arrivalWarehouse = (ImWarehouse) imWarehouseDAO.findByBrandCodeAndWarehouseCode(brandCode, arrivalWarehouseCode, "Y");// 轉入庫
			//ImWarehouse arrivalWarehouse = (ImWarehouse) imWarehouseDAO.findById(arrivalWarehouseCode); // 轉入庫
			
			if (null != arrivalWarehouse) {
				log.info("111"+arrivalWarehouse.getWarehouseManager());
			...
			// 轉入店別
			ImWarehouse arrivalStore = (ImWarehouse) imWarehouseDAO.findByBrandCodeAndWarehouseCode(brandCode, arrivalStoreCode,"Y");
	...
	private void setOriginalOrder(ImMovementHead imMovementHead, String identification, List errorMsgs) throws Exception {
		...
		try {
			...
			if (originalOrderTypeCode != null) {
				if (!StringUtils.hasText(imMovementHead.getOriginalOrderTypeCode())) {
					ImWarehouse imDeliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class,imMovementHead.getDeliveryWarehouseCode());
				//	ImWarehouse w7 = (ImWarehouse) imWarehouseDAO.findById(imMovementHead.getDeliveryWarehouseCode());
					//ImWarehouse imDeliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class,
						//	w7.getWarehouseId());
					if (null == imDeliveryWarehouse)
						...
						ImWarehouse imArrivalWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class,imMovementHead.getArrivalWarehouseCode());
						//ImWarehouse w8 = (ImWarehouse) imWarehouseDAO.findById(imMovementHead.getArrivalWarehouseCode());
						//ImWarehouse imArrivalWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class,
								//w8.getWarehouseId());
						if (null == imArrivalWarehouse)
							this.messageHandle(MessageStatus.LOG_ERROR, identification, "查無轉出倉別代號("
	...
	public List<Properties> executeModifyInitial(Map parameterMap) throws Exception {
		...
		try {
			...
			if (null != form) {
				...
				resultMap.put("originalArrivalDate", DateUtils
						.format(form.getArrivalDate(), DateUtils.C_DATE_PATTON_SLASH));
	...
	public HashMap updateDate(Map parameterMap) throws Exception {
		...
		try {
			...
			String arrivalDateString = (String) PropertyUtils.getProperty(otherBean, "arrivalDate");
			...
			log.info("arrivalDateString:" + arrivalDateString);
			Date deliveryDate = DateUtils.parseDate(DateUtils.C_DATE_PATTON_SLASH, deliveryDateString);
			Date arrivalDate = DateUtils.parseDate(DateUtils.C_DATE_PATTON_SLASH, arrivalDateString);
			...
			if (null != form) {
				if (StringUtils.hasText(form.getCmMovementNo())) {
					...
					if(form.getStatus().equals("CLOSE") || form.getStatus().equals("VOID")){
						throw new Exception("已 結案/作廢 之調撥單無法修改");
					} else {
	...
	public void aggregateLineBatchWAIT_IN(ImMovementHead head, Map aggregates) throws Exception {
		...
		for (Iterator iterator = items.iterator(); iterator.hasNext();) {
			...
			if (aggregateQuantity - deliveryQuantity > 0) {
				log.info("henry watch aggregateLineBatchWAIT_IN~~item.setArrivalQuantity"+deliveryQuantity);
				...
				log.info("henry watch aggregateLineBatchWAIT_IN~~else if (aggregateQuantity - deliveryQuantity == 0"+deliveryQuantity);
				item.setArrivalQuantity(deliveryQuantity);
				aggregates.remove(item.getItemCode());
			} else {
				log.info("henry watch aggregateLineBatchWAIT_IN~~aggregateQuantity"+aggregateQuantity);
		...
		for (Iterator iterator = keys.iterator(); iterator.hasNext();) {
			...
			log.info(" aggregates.get(object)"+ aggregates.get(object));
			log.info("henry watch this method (Double) aggregates.get(object)=="+(Double) aggregates.get(object));
	...
	public List<Properties> updateOriginalQtyToArrival(Map parameterMap) throws IllegalAccessException,
		...
		log.info("assignOriginalQtyToArrival....");
		Map returnMap = new HashMap(0);
		int startLineId = 0;
		try {
			Object otherBean = parameterMap.get("vatBeanOther");
			...
			if (null != formId) {
				...
				if (null != head) {
					...
					for (ImMovementItem item : items) {
						item.setDeliveryQuantity(item.getOriginalDeliveryQuantity());
						//item.setArrivalQuantity(item.getOriginalDeliveryQuantity());
						...
						ImSysLog ims = new ImSysLog();
						ims.setHeadId(head.getHeadId());
						ims.setAction("updateOriginalQtyToArrival");
						ims.setCreate_date(new Date());
						ims.setCreated_by(head.getCreatedBy());
						ims.setIdentification(head.getBrandCode()+head.getOrderTypeCode()+head.getOrderNo());
						ims.setMessage(item.getItemCode()+" DeliveryQuantity: "+item.getDeliveryQuantity());
						ims.setModule_name("Im_Movement");
						imSysLogDAO.merge(ims);
						ims=null;
	...
	public void executeMainDailyBalance(ImMovementHead imMovementHead, String opUser) throws Exception {
		...
		if (null == imMovementHead.getDeliveryDate()) {
			...
			ImWarehouse imDeliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class,imMovementHead.getDeliveryWarehouseCode());
			//ImWarehouse w9 = (ImWarehouse) imWarehouseDAO.findById(imMovementHead.getDeliveryWarehouseCode());
            //ImWarehouse imDeliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class,
					//w9.getWarehouseId());
			if (null != imDeliveryWarehouse) {
				cmDeliveryWarehouseCode = StringUtils.hasText(imDeliveryWarehouse.getCustomsWarehouseCode()) ? imDeliveryWarehouse
			...
			ImWarehouse imArrivalWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class, imMovementHead.getArrivalWarehouseCode());
			//ImWarehouse w10 = (ImWarehouse) imWarehouseDAO.findById(imMovementHead.getArrivalWarehouseCode());
			//ImWarehouse imArrivalWarehouse = (ImWarehouse) imWarehouseDAO.findByPrimaryKey(ImWarehouse.class, w10.getWarehouseId());
			...
			if (OrderStatus.CLOSE.equals(objMovement.getStatus()) || OrderStatus.FINISH.equals(objMovement.getStatus())) {
				...
				if (!StringUtils.hasText(objMovement.getCmMovementNo())) {
					...
					try {
						...
						if(OrderStatus.CLOSE.equals(objMovement.getStatus())){
							this.deleteTransection(objMovement);
						}
					...
					log.info("StringUtils.hasText(objMovement.getCmMovementNo())");
					ValidateUtil.isAfterClose(objMovement.getBrandCode(), objMovement.getOrderTypeCode(), "出貨日期",
							objMovement.getDeliveryDate(),objMovement.getSchedule());
					try {
						BuCommonPhraseLine orderTypeSetting = buCommonPhraseLineDAO.findById("ImMovementOrderType",
						...
						this.updateMoveOnHandtoVoid(objMovement, false, identification, errorMsgs, StringUtils
								.hasText(orderTypeSetting.getAttribute5()) ? orderTypeSetting.getAttribute5() : "NORMAL" , "WAIT_IN");
						if(OrderStatus.CLOSE.equals(objMovement.getStatus())){
							this.deleteTransection(objMovement);
						}
						objMovement.setStatus(OrderStatus.WAIT_IN);
						objMovement.setCreatedBy(owner);
						objMovement.setCmMovementNo("");
						objMovement.setProcessId(null); // 反轉時，清空PROCESS_ID
						log.info("調撥單反確資料更新");
						imMovementHeadDAO.update(objMovement);
						// log.info("調撥單反確流程重啟");
	...
	private void deleteTransection(ImMovementHead objMovement){
		List<ImTransation> imTransactions = imTransationDAO.findTransationByIdentification(
				objMovement.getBrandCode(), objMovement.getOrderTypeCode(), objMovement.getOrderNo());
		for(ImTransation imTransation:imTransactions){
	...
	public ImMovementHead createNewImMovement(ImMovementHead imMovementHead, Date date) throws Exception {
		...
		try {
			...
			//ImWarehouse arrivalWarehouse = (ImWarehouse) imWarehouseDAO.findById("ImWarehouse", imMovementHead
					//.getArrivalStoreCode());
			ImWarehouse arrivalWarehouse = (ImWarehouse) imWarehouseDAO.findByBrandCodeAndWarehouseCode(imMovementHead.getBrandCode(), imMovementHead.getArrivalWarehouseCode(), "Y");
			...
			if ("WHF".equals(imMovementHead.getOrderTypeCode()) || "WHP".equals(imMovementHead.getOrderTypeCode())) {// 贈品
				...
				newImMovementItem.setLotNo("00000000000"); // 批號
				...
				log.info("henry watch this method imMovementHead.getItemCount()(whf)"+imMovementHead.getItemCount());
	...
	public ImMovementHead createToNewImMovement(ImMovementHead imMovementHead, Map parameterMap) throws Exception {
		try {
			...
			BuLocationService buLocationService = (BuLocationService) SpringUtils.getApplicationContext().getBean(
					"buLocationService");
			//ImWarehouse w13 = imWarehouseDAO.findById(deliverWarehouseCode);
			//ImWarehouse deliveryWarehouse = (ImWarehouse) imWarehouseDAO.findById("ImWarehouse",deliverWarehouseCode);
			ImWarehouse deliveryWarehouse = (ImWarehouse) imWarehouseDAO.findByBrandCodeAndWarehouseCode(brandCode, deliverWarehouseCode, "Y");
			...
			// 取得轉入庫別的資料
			//ImWarehouse w14 = imWarehouseDAO.findById(arrivalWarehouseCode);
			//ImWarehouse arrivalWarehouse = (ImWarehouse) imWarehouseDAO.findById("ImWarehouse",arrivalWarehouseCode);
			ImWarehouse arrivalWarehouse = (ImWarehouse) imWarehouseDAO.findByBrandCodeAndWarehouseCode(imMovementHead.getBrandCode(), arrivalWarehouseCode, "Y");
			String customsArrivalWarehouseCode = "";
			BuLocation arrivalLocation = null;
			if (null != arrivalWarehouse) {
				customsArrivalWarehouseCode = arrivalWarehouse.getCustomsWarehouseCode();
			...
			for (int i = 0; i < imMovementItems.size(); i++) {
				...
				log.info("henry watch this method (ImMovementItem) imMovementItems.get(i)).getArrivalQuantity()"+((ImMovementItem) imMovementItems.get(i)).getArrivalQuantity());
	...
	public void updateStatus(Properties httpRequest){
		
		 Long headId = NumberUtils.getLong(httpRequest.getProperty("headId"));
		 String brandCode = httpRequest.getProperty("brandCode");
		 String status = httpRequest.getProperty("status");
		 
		 log.info("sssss:"+headId+" "+brandCode+" "+status);
		 ImMovementHead head = imMovementHeadDAO.findById(headId);
		 if(status.equals("cancel")){
			 head.setTranAllowUpload("D");
			 head.setTranRecordStatus("DF14");
			 imMovementHeadDAO.update(head);
		 }else{
			 head.setTranRecordStatus(status);
			 head.setTranAllowUpload("I");
			 head.setCustomsStatus("A");
			 head.setcStatus("R");
			 imMovementHeadDAO.update(head);
		 }
	...
	public void saveRpNo(ImMovementHead imMovementHead, String loginUser) throws NoSuchObjectException{
		System.out.println("取移倉申請書號碼開始");
		//MACO 2016.01.30 superviseCode
		String superviseCodeT2=buCommonPhraseLineDAO.findById("SuperviseCode","T2Warehouse").getName();
		String superviseCodeMS=buCommonPhraseLineDAO.findById("SuperviseCode","MSWarehouse").getName();
		try{
			Long seq = Long.valueOf(imMovementHeadDAO.getSendRqSq().get(0).toString());
			Calendar cl = Calendar.getInstance();
			cl.setTimeInMillis(System.currentTimeMillis());
			int year=cl.get(Calendar.YEAR)-1911;
			String RpNo = "T" + superviseCodeT2 + String.valueOf(year).substring(1) + String.format("%06d",seq) ;
			System.out.println("LLLLLLLLLLLLLLLLLLLLLLLL:"+RpNo);
			imMovementHead.setSendRpNo(RpNo);
			imMovementHead.setLastUpdatedBy(loginUser);//更新異動人員
			imMovementHead.setLastUpdateDate(new Date());//更新異動日期
			System.out.println("取移倉申請書號碼結束");
			System.out.println("更新移倉申請書號碼開始:更新單號以及最後更新日期及人員開始");
			
			imMovementHeadDAO.update(imMovementHead);//更新成正式單號
		}
		catch(Exception e)
		{
			log.info("取單號錯誤");
	...
	/*public List<Properties> updateIsUsed(Properties httpRequest){
		List list = new ArrayList();
		Properties properties = new Properties();
		try{
			Long headId = NumberUtils.getLong(httpRequest.getProperty("headId"));
			String usedStatus = httpRequest.getProperty("usedStatus")==null?"":httpRequest.getProperty("usedStatus");
			
			ImMovementHead head = imMovementHeadDAO.findById(headId);
			head.setIsUsed(usedStatus);
			log.info("head.getId:"+head.getHeadId()+",head.getusedStatus:"+head.getIsUsed());
			imMovementHeadDAO.save(head);
			ImMovementHead head2 = imMovementHeadDAO.findById(headId);
			if(head2.getIsUsed().equals("")){
				properties.setProperty("isUsed", head2.getIsUsed());
			}else{
				properties.setProperty("isUsed", head2.getIsUsed());
			}
	...
	public List<Properties> getCustomsProcessResponse(Properties httpRequest){
	...
	public static CustomsProcessResponseDAO getCustomsProcessResponseDAO() {
		return customsProcessResponseDAO;
	}

	public static void setCustomsProcessResponseDAO(
			CustomsProcessResponseDAO customsProcessResponseDAO) {
		ImMovementMainService.customsProcessResponseDAO = customsProcessResponseDAO;
	}
	//維修廠商資訊
	public List<Properties> getFixAddr(Map parameterMap) throws Exception {
		log.info("FixAddr....");
		Map returnMap = new HashMap(0);
		BuLocationService buLocationService = (BuLocationService) SpringUtils.getApplicationContext().getBean(
				"buLocationService");
		try {
			...
			String itemCategoryMode = (String) PropertyUtils.getProperty(otherBean, "itemCategoryMode");
			log.info("itemCategoryMode:" + itemCategoryMode);
			String fixSupplier = (String) PropertyUtils.getProperty(otherBean, "fixSupplier");
			log.info("fixSupplier:" + fixSupplier);
			/*String deliveryWarehouseCode = (String) PropertyUtils.getProperty(otherBean, "deliveryWarehouseCode");
			log.info("deliveryWarehouseCode:" + deliveryWarehouseCode);*/
			String arrivalWarehouseCode = (String) PropertyUtils.getProperty(otherBean, "arrivalWarehouseCode");
			log.info("arrivalWarehouseCode:" + arrivalWarehouseCode);
			...
			BuSupplierWithAddressView supplier = buSupplierWithAddressViewDAO.findById(fixSupplier, brandCode);
			log.info("維修廠商地址:"+supplier.getAAddress());
			
            if (null != supplier.getAAddress()) {
				
				if (null != supplier.getAAddress()) {
					returnMap.put("arrivalCity", StringUtils.hasText(supplier.getACity()) ? supplier.getACity()
							: "");
					returnMap.put("arrivalArea", StringUtils.hasText(supplier.getAArea()) ? supplier.getAArea()
							: "");
					returnMap.put("arrivalZipCode", StringUtils.hasText(supplier.getAZipCode()) ? supplier.getAZipCode() : "");
					returnMap.put("arrivalAddress", StringUtils.hasText(supplier.getAAddress()) ? supplier.getAAddress() : "");
					returnMap.put("customsArrivalWarehouseCode", "FW"); // 轉入店別倉庫
				} else {
					log.info("null");
					returnMap.put("arrivalCity", "");
					returnMap.put("arrivalArea", "");
	...
	public void updateCustomsStatus(Properties httpRequest) throws Exception {
	    ...
	    ImMovementHead im = (ImMovementHead)imMovementHeadDAO.findByPrimaryKey(CmMovementHead.class, headId1);
	    log.info("im申請書編號1："+im.getSendRpNo());
	    ...
	    if(tranRecordStatus.equals("cancel")){
	    	...
	    	String recordStatus = im.getTranRecordStatus();
	    	if(!recordStatus.equals("DF10")){
	    		im.setTranRecordStatus("DF15");
	    	}
	    	System.out.println("cm.getCustomsStatus():"+im.getCustomsStatus());
	    	if(im.getCustomsStatus().equals("N24") || im.getCustomsStatus().equals("N13")){
	    		im.setTranRecordStatus("DF10");
	    	}
	    	im.setTranAllowUpload("D");
	    	im.setCustomsStatus("A");
	    	imMovementHeadDAO.update(im);//更新上傳
	    }else{
	    	log.info("saveStatus");
		    im.setCustomsStatus("A");
	...
	public List<ImItemOnHandView> executeOnHandInitial(String loginBrandCode,String fitemCode) throws Exception {
		List<ImItemOnHandView> onHandQtyByItemCodes = null;
		try {
			
			//庫存查詢-By condition
			 onHandQtyByItemCodes = imItemOnHandViewDAO.getOnHandQtyListByItems(loginBrandCode, fitemCode);
			
		} catch (Exception ex) {
			ex.printStackTrace();
			log.error("庫存查詢失敗，原因：" + ex.toString());
		}
		return onHandQtyByItemCodes;
	}

	
	//判斷是否減合庫別權限 by jason 20160220
	public boolean isWarehouseExecute() throws Exception{
		log.info("isWarehouseExecute");
		boolean isWarehouseExecute = false;
		BuCommonPhraseLine buCommonPhraseLine = buCommonPhraseLineDAO.findOneBuCommonPhraseLine("execute", "warehouse","Y");
		log.info("isWarehouseExecute:"+isWarehouseExecute);
		if(buCommonPhraseLine == null)isWarehouseExecute = true;
		return isWarehouseExecute;
	    }
}