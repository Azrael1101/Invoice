...
public class BuPurchaseService {


	private static final Log log = LogFactory.getLog(BuPurchaseService.class);
	...
	public Map executeInitial(Map parameterMap) throws Exception{
		...
		try{
			//List<BuBrand> buBrands = formId != null ? buBrandDAO.findByProperty( "BuBrand", "brandCode", formId) : null; 
			...
			//List<AdCategoryHead> alltaskType = baseDAO.findByProperty( "AdCategoryHead", new String[] { "orderTypeCode","enable" },new Object[] {"AP","Y" },"displaySort");
			//List<AdCategoryLine> alltaskTypeNo = baseDAO.findByProperty( "AdCategoryLine", new String[] { "orderTypeCode","enable" },new Object[] {"AP","Y" },"displaySort");
			...
			//multiList.put("alltaskType"	, AjaxUtils.produceSelectorData(alltaskType, "groupNo", "groupName", false, false));
			//multiList.put("alltaskTypeNo"	, AjaxUtils.produceSelectorData(alltaskTypeNo, "classNo", "className", false, true));
	...
	throws FormException, Exception {
		...
		try {
			...
			buPurchase = !StringUtils.hasText(formIdString)? this.executeNewReturn(parameterMap): this.findById(formId) ;
	...
	public Map updateAJAXBuGoalAchevement(Map parameterMap) throws Exception {
		...
		BuPurchaseHead buPurchase = (BuPurchaseHead)parameterMap.get("entityBean");
		String createBy = buPurchase.getCreatedBy();
		...
		//String beforeStatus = buPurchase.getStatus();
		String nextStatus =  getStatus(orderTypeCode,formAction,buPurchase.getStatus());
		log.info("nextStatus~~~~~~~~~"+nextStatus);
		//log.info("現行動作:"+formAction);
		String brandCode      = (String)PropertyUtils.getProperty(otherBean, "brandCode");	
		//發信用
		String subject = null;
		...
		try {
			...
			if(orderTypeCode.equals("LOA")&&(String) PropertyUtils.getProperty(formBindBean, "costControl")!=null&&(String) PropertyUtils.getProperty(formBindBean, "warehouseControl")!=null){

				BuEmployee emp = buEmployeeDAO.findById((String) PropertyUtils.getProperty(formBindBean, "applicant"));
				
            			

				//BuEmployee emp = buEmployeeDAO.findById(role);
				if(null==emp){
		                    emp.setEmployeeRole(emp.getEmployeeCode());
		                }
				//log.info("申請修改成本:"+role+" "+emp);

				log.info("申請人為:"+emp.getEmployeeCode());
				
				emp.setWarehouseControl(buPurchase.getWarehouseControl());
				emp.setCostControl(buPurchase.getCostControl());
				
				
				if(null==emp.getSwitchTo()){
					emp.setSwitchTo("NEW");
				}
				else if(emp.getSwitchTo().equals("OLD")){
					emp.setSwitchTo("NEW");
				}
				
				if(null==emp.getEMailCompany()){
					emp.setEMailCompany(emp.getEmployeeCode()+"@tasameng.com.tw");
				}
				buEmployeeDAO.merge(emp);


				if(null==buEmployeeDAO.findById((String) PropertyUtils.getProperty(formBindBean, "applicant")).getEmployeeRole()){
log.info("1111111111");
					BuEmployee emp1 = buEmployeeDAO.findById((String) PropertyUtils.getProperty(formBindBean, "applicant"));
					emp1.setEmployeeRole(emp1.getEmployeeCode());
					buEmployeeDAO.update(emp1);
					emp1.setWarehouseControl(buPurchase.getWarehouseControl());
					emp1.setCostControl(buPurchase.getCostControl());
					if(null==emp1.getSwitchTo()){
						emp1.setSwitchTo("NEW");
					}else if(emp1.getSwitchTo().equals("OLD")){
						emp1.setSwitchTo("NEW");
					}
					if(null==emp1.getEMailCompany()){
						emp1.setEMailCompany(emp.getEmployeeCode()+"@tasameng.com.tw");
					}
					buEmployeeDAO.merge(emp1);
				}else{
					log.info("22222222");
					if(null==emp.getSwitchTo()){
						emp.setSwitchTo("NEW");
			...
			if(buPurchase.getOrderTypeCode().equals("LOA")&&formAction.equals("SIGNING")){ 
				log.info("進入簽核SIGNING~");
				buPurchase.setStatus("SIGNING");
				
				
				
				getAccessMailList(buPurchase,subject, templateFileName, displayHteml, mailAddress, attachFiles, cidMap,formAction);

			}else if((buPurchase.getOrderTypeCode().equals("LOA")&&formAction.equals("SUBMIT"))|buPurchase.getOrderTypeCode().equals("WOA")){
				log.info("進入SUBMIT");
				//log.info("costControl~~~~~~~"+(String) PropertyUtils.getProperty(formBindBean, "costControl"));
				//log.info("warehouseControl~~~~~~~"+(String) PropertyUtils.getProperty(formBindBean, "warehouseControl"));
				BuEmployee emp = buEmployeeDAO.findById((String) PropertyUtils.getProperty(formBindBean, "applicant"));
				if(buPurchase.getOrderTypeCode().equals("LOA")){
					buPurchase.setStatus("FINISH");	
				}

				log.info("測試role:"+emp.getEmployeeRole());
				Long a = competenceService.updateCompetenceData(buPurchase.getHeadId(),loginEmployeeCode,emp.getEmployeeRole(),brandCode);
		
				
				getAccessMailList(buPurchase,subject, templateFileName, displayHteml, mailAddress, attachFiles, cidMap,formAction);
			}else if(buPurchase.getOrderTypeCode().equals("LOA")&&formAction.equals("SAVE")){
				log.info("進入SAVE");
				buPurchase.setStatus("SAVE");	
			}

			if(formStatus.equals(OrderStatus.REJECT))
				nextStatus = OrderStatus.SAVE;
			/*if(formStatus.equals(OrderStatus.PLAN))
				nextStatus = OrderStatus.FINISH;*/
			/*else if(formStatus.equals(OrderStatus.PLAN))
				nextStatus = OrderStatus.SUSPEND;
			else if(formStatus.equals(OrderStatus.SUSPEND))
				nextStatus = OrderStatus.PLAN;*/
			if(buPurchase.getOrderTypeCode().equals("IRQ")&&buPurchase.getStatus().equals(OrderStatus.SIGNING))
			{
				getAutoMailList(buPurchase,subject, templateFileName, displayHteml, mailAddress, attachFiles, cidMap,formAction,createBy);
			}
			...
			if(!buPurchase.getOrderTypeCode().equals("LOA")){
				buPurchase.setStatus(nextStatus);
			}
			...
			buPurchaseHeadDAO.merge(buPurchase);
	 ...
	 * @throws Exception
	 */ 
	public static Object[] startProcess(BuPurchaseHead form,String version,String processId) throws Exception {
		log.info("startProcess");
		try {
			String packageId = "Bu_Purchase";
			//String processId = "process";
			//String version = "20140310";
			String sourceReferenceType = "BuPurchaseHead (1)";
	...
	//reprocess 簽核
	public static Object[] reAssignment(Long assignmentId, Long processId,String loginName,BuPurchaseHead buPurchaseHead) throws Exception {
		try {
	...
	throws FormException, Exception {
		...
		try {			
			...
			/*if(buPurchaseHead ==null || formId == 0){
				this.executeNewReturn(parameterMap);
			}else{
				this.findById(formId) ;	
			}*/
...
//			findObjs.put(" or model.creatBy = :loginEmployeeCode", loginEmployeeCode);
	...
	NoSuchMethodException,Exception {
		...
		try{
			...
			log.info("adDetail.headId" + headId);
			List adDetail = adDetailDAO.findPageLine(headId, iSPage, iPSize);
			List buShop  = buShopDAO.findPageLine(employeeCode,iSPage, iPSize);
			log.info("adDetail"+adDetail);
			log.info("buPurchases.size"+ adDetail.size());	
			if(buShop != null && buShop.size() > 0){
				//Long indexNo = 1L;
				BuShop b = (BuShop)buShop.get(0);
				//Long firstIndex = b.getIndexNo();
				Long maxIndex = buShopDAO.findPageLineCount(employeeCode);
				Long firstIndex =Long.valueOf(iSPage * iPSize)+ 1;  */
	//b.setIndexNo(indexNo);
	//indexNo++;
	//b.getBrandCode();
	...
	/*for(int i = 0; i<buShop.size(); i++){
					BuShop m = (BuShop)buShop.get(i);
					m.setIndexNo(Long.parseLong((firstIndex+i) + ""));
				}
				re.add(AjaxUtils.getAJAXPageData(httpRequest,GRID_SEARCH_FIELD_NAMESSSHOP, GRID_FIELD_VALUES_KWE_SHOP, buShop, gridDatas,firstIndex,maxIndex));
			}
			if (adDetail != null && adDetail.size() > 0) {
				// 設定額外欄位
				//this.setBuPurchaseLineOtherColumn(buPurchaseLines);
				...
				log.info("IIIndex:"+firstIndex+","+maxIndex);
				// 取得最後一筆 INDEX
				re.add(AjaxUtils.getAJAXPageData(httpRequest,GRID_FIELD_NAMES_KWE_SHOP, GRID_FIELD_VALUES_KWE_SHOP, adDetail, gridDatas, firstIndex, maxIndex));
			}else {
				re.add(AjaxUtils.getAJAXPageDataDefault(httpRequest,GRID_FIELD_NAMES_KWE_SHOP, GRID_FIELD_VALUES_KWE_SHOP,  gridDatas));
			}
			return re;
		}catch(Exception ex){
			log.error("載入頁面顯示的bupurchase發生錯誤，原因：" + ex.toString());
