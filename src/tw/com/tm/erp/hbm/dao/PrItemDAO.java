...
public class PrItemDAO extends BaseDAO {
    private static final Log log = LogFactory.getLog(PrItemDAO.class);
    ...
    public List<PrItem> findAll() {
        log.debug("finding all PrItem instances");
        try {
            String queryString = "from PrItem";
    ...
    public List<PrItem> find(HashMap conditionMap) {
        ...
        List<PrItem> result = getHibernateTemplate().executeFind(
                new HibernateCallback() {
                    public Object doInHibernate(Session session)
                            throws HibernateException, SQLException {

                        StringBuffer hql = new StringBuffer("select model from PrItem as model");
	...
	public Long getMaxIndexNo(Long headId) {
		...
		List result = getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
				...
				hql.append("select MAX(model.indexNo) as indexNo from PrItem as model where 1=1 ");
    ...
    public  List<PrItem>  findLineByDeliveryOrderNo(Long headId, String orderTypeCode, String deliveryOrderNo) {
		...
		List<PrItem> result = getHibernateTemplate().executeFind(new HibernateCallback() {
			public List<PrItem> doInHibernate(Session session) throws HibernateException, SQLException {
				StringBuffer hql = new StringBuffer("");
				hql.append("from PrItem as model where 1=1 ");
		...
		log.info("PrItem.form:" + result.size());
		return result;
	}

    public PrItem findByLocationName(Long orderTypeCode) {

            StringBuffer hql = new StringBuffer("from PrDetail as model ");
            hql.append("where model.orderTypeCode = ? ");
            List<PrItem> lists = getHibernateTemplate().find(hql.toString(),
                    new Object[] { orderTypeCode });
            return (lists != null && lists.size() > 0 ? lists.get(0) : null);
        }
    
    public List<PrItem> findPageLine(Long headId, int startPage, int pageSize) {
		log.info("BuPurchaseLineDAO.findPageLine headId=" + headId + "startPage=" + startPage + "pageSize" + pageSize);
		final int startRecordIndexStar = startPage * pageSize;
		final int pSize = pageSize;
		final Long hId = headId;
		List<PrItem> re = getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException, SQLException {
	...
	public PrItem findItemByIdentification(Long headId, Long lineId) {
		StringBuffer hql = new StringBuffer(
				"from BuPurchaseLine as model where model.buPurchaseHead.headId = ? and model.lineId = ?");
		List<PrItem> result = getHibernateTemplate().find(hql.toString(), new Object[] { headId, lineId });
		return (result != null && result.size() > 0 ? result.get(0) : null);
	}
	
	public PrItem findItem(String itemNo) {
		log.info("~~~~~~~~~~~~~~~");
	StringBuffer hql = new StringBuffer("from PrItem as model where 1=1 ");
	//hql.append(" and model.brandCode = ? ");
	hql.append(" and model.itemNo  = ? ");
	log.info("~~~~~~~~~~~~~~~1");
	List<PrItem> lists = getHibernateTemplate().find(hql.toString(), new String[] { itemNo });
	log.info("~~~~~~~~~~~~~~~2");
	...
	public PrItem findById(java.lang.String id) {
		if(StringUtils.hasText(id)){
		    id = id.toUpperCase();
		}
		final String itemNo = id;
		List<PrItem> result = getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException, SQLException {

				StringBuffer hql = new StringBuffer("from PrItem as model where 1=1 ");
				hql.append(" and model.itemNo  = :itemNo ");
	...
	public PrItem findItemName(String itemName) {
		log.info("~~~~~~~~~~~~~~~");
	StringBuffer hql = new StringBuffer("from PrItem as model where 1=1 ");
	//hql.append(" and model.supplierNo = model1.supplierNo ");
	hql.append(" and model.itemName  = ? ");
	log.info("~~~~~~~~~~~~~~~1");
	List<PrItem> lists = getHibernateTemplate().find(hql.toString(), new String[] { itemName });
	log.info("~~~~~~~~~~~~~~~2");
	if ((null != lists) && (lists.size() > 0))
		return lists.get(0);
	else
	...
	public PrItem findByItemName(String itemName) {

		final String itemName_CName = itemName;
		List<PrItem> result = getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException, SQLException {

				StringBuffer hql = new StringBuffer("from PrItem as model where 1=1 ");
				hql.append(" and model.itemName  = :itemName ");

				Query query = session.createQuery(hql.toString());
				query.setString("itemName", itemName_CName);
	...
	public List<PrItem> findByCategory(String category02,String itemDefault) {
	    log.info("category02:"+category02);
		StringBuffer hql = new StringBuffer(" from PrItem as model");
		hql.append(" where model.id.category02 = ? ");
		hql.append(" and model.id.itemDefault = ? ");
		List<PrItem> prItem = getHibernateTemplate()
			.find(hql.toString(), new String[] { category02 ,itemDefault});
    ...
    public HashMap findPageLine(HashMap findObjs, int startPage,  int pageSize, String searchType) {
    	...
    	List result = getHibernateTemplate().executeFind(new HibernateCallback() {
    		public Object doInHibernate(Session session)throws HibernateException, SQLException {
    			...
    			String itemNo   = (String) fos.get("itemNo");
    			String itemName     = (String) fos.get("itemName");
    			//String itemDefault     = (String) fos.get("itemDefault");

    			    			   			
    			StringBuffer hql = new StringBuffer("");
    			log.info("findPageLine~~~~~~~~~~~~~~@@@@@@@@@@@@@@@@@@");
    			if(QUARY_TYPE_RECORD_COUNT.equals(type))
    				hql.append(" select count(model.itemId) as rowCount from PrItem as model where 1=1 ");
    			else if(QUARY_TYPE_SELECT_ALL.equals(type))
    				hql.append(" select model.itemId from PrItem as model where 1=1 ");
    			else
    				hql.append(" from PrItem as model where 1=1 ");
    			...
    			//if (StringUtils.hasText(itemDefault))
    				//hql.append(" and model.itemDefault = :itemDefault");	
    		
	
     			hql.append(" order by model.itemNo");
    			...
    			if (StringUtils.hasText(itemNo))
    				query.setString("itemCode", itemNo);

    			if (StringUtils.hasText(itemName))
    				query.setString("itemName", itemName);

    			//if (StringUtils.hasText(itemDefault))
    				//query.setString("itemDefault", itemDefault);
