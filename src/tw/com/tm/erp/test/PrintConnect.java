...
public class PrintConnect {
	...
	public PrintConnect (){
		System.out.println(" initial PrintConnect() ... ");
	}
	
	public Connection getSaleConnection() {
		
		try {
            if(conn != null) {
            	System.out.println("MySQL已連線...");
            }else {
            	Class.forName("com.mysql.jdbc.Driver");
	            conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/tasameng?serverTimezone=CST", "root", "");
...
//        Statement stmt = getSaleConnection().createStatement();
...
//        ResultSet rs = stmt.executeQuery("Select * from salehead_table");
//        while(rs.next()) {     
