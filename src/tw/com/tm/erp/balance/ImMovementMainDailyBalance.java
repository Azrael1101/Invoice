...
public class ImMovementMainDailyBalance implements DailyBlanceInterface {
	private static final Log log = LogFactory.getLog(ImMovementMainDailyBalance.class);
	...
	try{
	    ...
	    for(int i = 0; i < brandCodeArray.length; i++){
			//出貨單結算	 
			List<ImMovementHead> imMovementHeads = imMovementHeadDAO.findMovementByProperty(brandCodeArray[i], OrderStatus.FINISH, startDate, endDate);
			...
			if(imMovementHeads != null && imMovementHeads.size() > 0){
				System.out.println("調撥單模組結算預計執行"+imMovementHeads.size()+"筆");
			    for(int j = 0; j < imMovementHeads.size(); j++){
					...
					try{
					    ...
					    System.out.println(identification+"("+(j+1)+"/"+imMovementHeads.size()+")");
					    imMovementService.executeMainDailyBalance(movementHead, opUser);
					}catch(Exception ex){
