...
vat.block.create(vnB_master = 3, {
	...
	rows:[  
	 ...
	 {items:[{name:"#L.totalUnitPriceAmount"    , type:"LABEL" , value:"本次使用預算"}]},	 
	 ...
	 {items:[{name:"#F.totalUnitPriceAmount"    , type:"TEXT",  bind:"totalUnitPriceAmount", size:12, mode:"READONLY",ceap:"#form.totalUnitPriceAmount"}]},
...
function detailInitial() {
	...
	vat.item.make(vnB_Detail, "quantity", {type:"NUMB", size:8, maxLen:20, desc:"數量", onchange:"calculateLineAmount()"});    
	vat.item.make(vnB_Detail, "foreignUnitCost", {type:"NUMB", size:8, maxLen:20, desc:"原幣單價", onchange:"calculateLineAmount()"});
	...
	vat.item.make(vnB_Detail, "actualPurchaseQuantity", {type:"NUMB", size:8, maxLen:20, desc:"實際應到貨數量", mode:"HIDDEN"});           
...
function execSubmitAction(actionId){
    ...
    //alert("approvalResult=" + approvalResult);
        ...
        if("SUBMIT_BG" == actionId){
            vat.block.submit(function(){return "process_object_name=poPurchaseAction"+
...
function exportFormData(){
    ...
    var url = "/erp/jsp/ExportFormData.jsp" + 
              ...
              "&fileType=XLS" + 
              "&processObjectName=poPurchaseOrderHeadService" + 
