...
function searchInitial(){ 
  ...
  if(typeof document.forms[0]["#loginBrandCode"] != 'undefined'){
     vat.bean().vatBeanOther = 
		...
		vat.bean.init(	
	  		function(){
				return "process_object_name=buGoalAction&process_object_method_name=performSearchInitial"; 
	    	},{								
	    		other: true
    	}); 
  }
...
vat.block.create( vnB_Header, {
	...
	title:"業績目標查詢作業", rows:[  
	 		{row_style:"", cols:[
				...
				{items:[{name:"#F.status", 				type:"SELECT" , init:allStatus , size:1}]}
			]}
		], 	 
		beginService:"",
		closeService:""			
...
function loadBeforeAjxService(){
   // alert("loadBeforeAjxService");	
	...
	var processString = "process_object_name=buGoalService&process_object_method_name=getAJAXSearchPageData" + 
...
function saveBeforeAjxService() {
	...
	var processString = "";
	if (checkEnableLine()) {
		processString = "process_object_name=buGoalService&process_object_method_name=saveSearchResult";
	}
	return processString;
}								
...
function openModifyPage(){
	...
	if(!(vFormId == "" || vFormId == "0")){
    	var url = "/erp/Bu_Goal:create:20091112.page?formId=" + vFormId; 
