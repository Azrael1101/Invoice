	...
	title:"", rows:[  
	 {row_style:"", cols:[
	 	{items:[{name:"#B.search"      , type:"IMG"      , value:"查詢",  src:"./images/button_find.gif", eClick:"doSearch()"},
	 			...
	 			{name:"SPACE"          , type:"LABEL"    , value:"　"},
	 			{name:"#B.export2"	   , type:"IMG"      ,value:"明細匯出",   src:"./images/button_detail_export.gif", eClick:'doExport2()'},
...
function kweImHeader(){ 
	...
	title:"商品資料查詢作業", rows:[  
		   ...
		   {row_style:( brandCode.indexOf("T2") > -1 ? "" : " style= 'display:none;'"), cols:[ 
				 ...
				 {items:[{name:"#F.foreignCategory", 	type:"TEXT", 	bind:"foreignCategory" },
				 		 {name:"#F.costControl", 			type:"hidden", 	bind:"costControl"}],td:"colSpan=7"}
			]}
			/*,
...
function doFormAccessControl(){
	var brandCode = vat.bean().vatBeanOther.loginBrandCode;
	var costControl = vat.item.getValueByName("#F.costControl");
	if( brandCode.indexOf("T2") <= -1){
		vat.item.setStyleByName("#B.export", 		"display", "none");
	}
	
	if(costControl != "Y"){
		vat.item.setStyleByName("#B.export2", 		"display", "none");
	}
...
function doExport2(){
    ...
    var url = "/erp/jsp/ExportFormView.jsp" + 
              "?exportBeanName=IM_ITEM2" + 
              "&fileType=XLS" + 
              "&processObjectName=imItemService" + 
              "&processObjectMethodName=getAJAXExportDataForT2B" + 
              "&brandCode" 			+ "=" + vat.bean().vatBeanOther.loginBrandCode +     
	          "&startItemCode"     	+ "=" + vat.item.getValueByName("#F.startItemCode"   ) +     
	          "&endItemCode"   		+ "=" + vat.item.getValueByName("#F.endItemCode" ) +     
	          "&itemCodes"   		+ "=" + vat.item.getValueByName("#F.itemCodes" ) +
