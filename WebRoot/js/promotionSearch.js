...
function formDataInitial(){ 
  if(typeof document.forms[0]["#loginBrandCode"] != 'undefined'){
     ...
     vat.bean.init(function(){
		return "process_object_name=imPromotionMainAction&process_object_method_name=performSearchInitial"; 
     },{other: true});
...
vat.block.create(vnB_Header, {
	...
	title:"促銷活動查詢作業", rows:[  
	 ...
	 {row_style:"", cols:[
	 	...
	 	{items:[{name:"#F.itemCategory",  type:"SELECT", bind:"itemCategory", size:12, init:allItemCategory, mode:function(){if("T2" == document.forms[0]["#loginBrandCode"].value)return "";else return "READONLY"}()}]},
	 ...
	 {row_style:"", cols:[
	 	...
	 	{items:[{name:"#F.itemCode",  type:"TEXT", bind:"itemCode", size:20}],td:""},
	 	...
	 	{items:[{name:"#F.category02",  type:"TEXT", bind:"category02", size:20, onchange:"upperCase('#F.category02')"}],td:" colSpan=4"}]},
	 	
	 {row_style:"", cols:[
	 	{items:[{name:"#L.category17",  type:"LABEL",  value:"廠商代號"}]},	 
	 	{items:[{name:"#F.category17",  type:"TEXT", bind:"category17", size:20, onchange:"upperCase('#F.category17')"}],td:" colSpan=6"}]}
	 ], 
...
function detailInitial(){
	...
	vat.item.make(vnB_Detail, "beginDate",          {type:"TEXT", view:"",   size:10, mode:"READONLY", desc:"啟始日期"});
	...
	vat.item.make(vnB_Detail, "originalPrice",         		 {type:"TEXT", view:"shift",   size:10, mode:"READONLY", desc:"定價"});
	vat.item.make(vnB_Detail, "totalDiscountAmount",         {type:"TEXT", view:"shift",   size:10, mode:"READONLY", desc:"促銷價"});
	vat.item.make(vnB_Detail, "totalDiscountPercentage",     {type:"TEXT", view:"shift",   size:10, mode:"READONLY", desc:"促銷率"});
...
function loadBeforeAjxService(){
	var startDate = getStartDate();
	var processString = "process_object_name=imPromotionMainService&process_object_method_name=getAJAXSearchPageData" + 
...
function saveBeforeAjxService() {
	var processString = "";
	processString = "process_object_name=imPromotionMainService&process_object_method_name=saveSearchResult";
	return processString;
}
...
function doClosePicker(){
	vat.block.pageSearch(2, {funcSuccess : function(){
		vat.block.submit(
			function(){ return "process_object_name=imPromotionMainAction&process_object_method_name=performSearchSelection";
