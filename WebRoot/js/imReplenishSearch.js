...
function searchInitial(){ 
  ...
  if(typeof document.forms[0]["#loginBrandCode"] != 'undefined'){
	...
	vat.bean.init(	
  		function(){
			return "process_object_name=imReplenishAction&process_object_method_name=performSearchInitial"; 
    	},{
    		other: true
   	});   
  }
...
vat.block.create( vnB_Header, {
	...
	title:"自動補貨查詢作業", rows:[  
	 		{row_style:"", cols:[
				...
				{items:[{name:"#L.warehouseArea", 		type:"LABEL"  , value:"倉庫區域"}]},
				{items:[{name:"#F.warehouseArea", 		type:"TEXT"   , size:20}]},
...
function detailInitial(){
	...
	vat.item.make(vnB_Detail, "warehouseName"   	, {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"庫別名稱"      });
	vat.item.make(vnB_Detail, "warehouseArea"   	, {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"倉庫區域"      });
	vat.item.make(vnB_Detail, "creationDate"   		, {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"建檔日期"      });
	vat.item.make(vnB_Detail, "createdNameBy"   	, {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"建檔人員"      });
	vat.item.make(vnB_Detail, "lastUpdateDate"   	, {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"修改日期"      });
	vat.item.make(vnB_Detail, "lastUpdatedNameBy"   , {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"修改人員"      });
...
function loadBeforeAjxService(){
	...
	var processString = "process_object_name=imReplenishService&process_object_method_name=getAJAXSearchPageData" + 
                      "&loginBrandCode"         + "=" + vat.bean().vatBeanOther.loginBrandCode +     
                      "&warehouseCode"         + "=" + vat.item.getValueByName("#F.warehouseCode" ) +     
	                  "&warehouseArea"         + "=" + vat.item.getValueByName("#F.warehouseArea" );     
...
function saveBeforeAjxService() {
	var processString = "";
	if (checkEnableLine()) {
		processString = "process_object_name=imReplenishService&process_object_method_name=saveSearchResult";
	}
	return processString;
}								
...
function doClosePicker(){
	   vat.block.pageSearch(vnB_Detail, {
    		funcSuccess : 
    		    function(){
    		      vat.block.submit(
    		                     function(){ return "process_object_name=imReplenishService&process_object_method_name=getSearchSelection";
...
function resetForm(){
    ...
    vat.item.setValueByName("#F.warehouseArea", "");
}
...
function openModifyPage(){
    ...
    var url = "/erp/Im_Replenish:create:20100607.page?formId=" + vFormId; 
     ...
     sc=window.open(url, '自動補貨維護作業', 'menubar=no,resizable=no,scrollbars=no,status=no,resizable=yes,scrollbars=yes');
