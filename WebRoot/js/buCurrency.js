				 	...
				 	cols : [
					 	{
					 		items : [
					 			...
					 			{name : "SPACE", type : "LABEL", value : "　"},
					 			{
					 				...
					 				service : "Bu_Currency:search:20131021.page", left : 0, right : 0, width : 1024, height : 768,	
			...
			rows : [
				{
					row_style : "", 
					cols : [
						{items : [{name : "#L.currencyCode", type : "LABEL", value : "幣別代碼<font color='red'>*</font>"}]},
						{items : [{name : "#F.currencyCode", type : "TEXT", bind : "currencyCode", size : 3, maxLen : 3}]},
						{
							items : [{name : "#F.enable", type : "CHECKBOX",  bind : "enable"}, {name : "#L.enable", type : "LABEL", value : "停用?", size : 10}],
							td : "colSpan=2"
						}
					]
				},
				{
					row_style : "", 
					cols : [
						{items : [{name : "#L.currencyCName", type : "LABEL", value : "中文名稱<font color='red'>*</font>"}]},
						{items : [{name : "#F.currencyCName", type : "TEXT", bind : "currencyCName", size : 25, maxLen : 25}]},
						{
							items : [{name : "#L.description", type : "LABEL", value : "說明", row : 2, col : 40}], 
							td : "rowSpan=2"
						},
						{
							items : [{name : "#F.description", type : "TEXTAREA", bind : "description", size : 50, row : 2, col : 50}], 
							td : "rowSpan=2"
						}
					]
				},
				{
					row_style : "", 
					cols : [
						{items : [{name : "#L.currencyEName", type : "LABEL", value : "英文名稱<font color='red'>*</font>"}]},
						{items : [{name : "#F.currencyEName", type : "TEXT", bind : "currencyEName", size : 25, maxLen : 25}]}
					]
				},
				{
					row_style : "", 
					cols : [
						...
						{items : [{name : "#F.orders", type : "TEXT", bind : "orders" , size : 2, maxLen : 2}]},
						{items : [{name : "#L.sourceCurrency", type : "LABEL", value : "找零幣別<font color='red'>*</font>"}]},
						{items : [{name : "#F.sourceCurrency", type : "SELECT", bind : "exChangeCode"}]}
				...
				},
				{
					...
					cols : [
						...
						{items : [{name : "#L.otherCurrency", type : "LABEL", value : "其他幣別<font color='red'>*</font>"}]},
						{items : [{name : "#F.otherCurrency", type : "SELECT", bind : "otherCurrency" , init:vaOpt1}]},
						{items : [{name : "#L.note", type : "LABEL", value : "<font color='red'>*</font>為必填欄位，請務必填寫。"}],td : "colSpan=2"}
					]
				}		
			], 		 
			beginService : "",
...
function doSubmit(formAction){
	...
	if(confirm(alertMessage)){
		...
		vat.block.submit(
			function(){
				return "process_object_name=buBasicDataAction" + "&process_object_method_name=performBuCurrencyTransaction";
			},
...
function doAfterPickerProcess(){
	if(vat.bean().vatBeanPicker.result != null){
		...
		if(vsMaxSize === 0){
			...
			var code = vat.bean().vatBeanPicker.result[vat.bean().vatBeanOther.currentRecordNumber - 1].currencyCode;
			refreshForm(code);
		}
		vat.item.setValueByName("#L.currentRecord", vat.bean().vatBeanOther.currentRecordNumber);
		vat.item.setValueByName("#L.maxRecord", vat.bean().vatBeanOther.lastRecordNumber);
