...
function kweImSearchInitial(){
  if(typeof document.forms[0]["#loginBrandCode"] != 'undefined'){
  	 ...
  	 if(document.forms[0]["#orderTypeCode"].value == null || document.forms[0]["#orderTypeCode"].value == ""){
  		// 新增T2A的品牌
  	 	orderTypeCode = "T2" == document.forms[0]["#loginBrandCode"].value ? "WTF": "T2B"==document.forms[0]["#loginBrandCode"].value ? "ETR" : "IMV";
  	 }else{
...
function loadBeforeAjxService(){
	...
	var processString = "process_object_name=imMovementService&process_object_method_name=getAJAXSearchPageData" +
                      ...
                      "&customsWarehouseCode"   + "=" + vat.bean().vatBeanOther.customsWarehouseCode +
...
function selectAll(){
  ...
  if(isProcessing)
  	processString = "process_object_name=imMovementService&process_object_method_name=updateAllSearchData";
  return processString;
...
function modifyDeliveryDate(){
	...
	var returnData = window.showModalDialog(
		...
		"?formId=" + vFormId ,"",
		"dialogHeight:360px;dialogWidth:400px;dialogTop:100px;dialogLeft:100px;");
}
...
function openModifyPage(){
    ...
    if(null != vFormId && "" != vFormId && 0 != vFormId){
    	if(vOrderTypeCode==="RSF"){
    		var url = "/erp/Im_Movement:create:20141023.page?formId=" + vFormId;
    	}else{
    	...
    	function(){
    		return "process_object_name=tmpAjaxSearchDataService"+
			       ...
			       {other: true,
					...
					vat.block.submit(
						function(){
							return "process_object_name=imMovementService&process_object_method_name=getAJAXWarehouseInfoByOrderType";
				     	},{other : true,
