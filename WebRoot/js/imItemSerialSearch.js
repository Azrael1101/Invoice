...
function saveBeforeAjxService() {
	...
	if (checkEnableLine()) {
		processString = "process_object_name=imItemService&process_object_method_name=saveSerialSearchResult";
		//alert(processString);
	}
...
function resetForm(){
    ...
    vat.item.setValueByName("#F.isUsed", "");
}
...
function openModifyPage(){
    ...
    var url = "/erp/Im_Item:createSerial:20100404.page?formId=" + vFormId; 

     sc=window.open(url, '商品序號維護作業', 'menubar=no,resizable=no,scrollbars=no,status=no,resizable=yes,scrollbars=yes');
