...
if(vat.bean().vatBeanOther.accessType == "SIMPLE"){
	...
	vat.block.create(vnB_Header = 1, {
		...
		title:"入提單查詢作業", rows:[  
		 ...
		 {items:[{name:"#F.transactionSeqNo"         , type:"TEXT",  bind:"transactionSeqNo", size:10}]},
...
function kweDetail(){
	...
	vat.item.make(vnB_Detail, "transactionSeqNoString"    , {type:"TEXT" ,view:"", size: 9,  mode:"READONLY", desc:"交易序號" , alter:true  });
...
function loadBeforeAjxService(){
    ...
    if(vat.bean().vatBeanOther.accessType == "SIMPLE"){
    	...
    	if(vat.item.getValueByName("#F.orderNo").substring(0, 3)=="DKP"){
...
function openModifyPage(){
    ...
    if(null != vFormId && "" != vFormId && 0 != vFormId){
    	var url = "/erp/So_Delivery:edit:20101014.page?formId=" + vFormId;	
...
function openReportWindow() {
    ...
    if(vbAllow){
	    ...
	    vat.bean().vatBeanOther.cwCode = "HD"
