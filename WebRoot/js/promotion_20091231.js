...
function outlineBlock(){
  ...
  if(typeof document.forms[0]["#loginBrandCode"] != 'undefined' && document.forms[0]["#loginBrandCode"].value != "[binding]"){
	vat.bean().vatBeanOther =
    {
	 ...
	 return "process_object_name=imPromotionMainAction&process_object_method_name=performInitial"; 
       },{other: true});
  }
  ...
  if (typeof vat.tabm != 'undefined') {
      ...
      vat.tabm.createButton(0, "xTab2","參與專櫃"    ,"vatShopDiv"              ,"images/tab_shop_data_dark.gif"            ,"images/tab_shop_data_light.gif", false);
      vat.tabm.createButton(0, "xTab3","客戶類別"    ,"vatCustomerDiv"          ,"images/tab_customer_type_dark.gif"        ,"images/tab_customer_type_light.gif", false);
      vat.tabm.createButton(0, "xTab4","金額統計"    ,"vatAmountDiv"            ,"images/tab_total_amount_dark.gif"         ,"images/tab_total_amount_light.gif", false , "showTotalCountPage()" );
	 ...
	 {row_style:"", cols:[
	 	{items:[{name:"#B.new"         , type:"IMG"    , value:"新增",  src:"./images/button_create.gif", eClick:"createNewForm()"},
	 			...
	 			{name:"#B.search"      , type:"PICKER" , value:"查詢",  src:"./images/button_find.gif"  , 
	 									 openMode:"open", 
	 									 service:"Im_Promotion:search:20091231.page",
...
vat.block.create(vnB_Header, {
	...
	title:"促銷活動維護作業", rows:[  
	 ...
	 {items:[{name:"#F.priceType", type:"SELECT",  bind:"priceType", size:1}]},			 
...
function detailInitial(){
	...
	vat.item.make(vnB_Detail, "itemCode", {type:"TEXT", view:"fixed", size:15, maxLen:20, desc:"品號", mask:"CCCCCCCCCCCC", onchange:"changeItemData()"}); 
	...
	vat.item.make(vnB_Detail, "itemName", {type:"TEXT", view:"", size:18, maxLen:20, desc:"品名", mode:"READONLY"});   
	vat.item.make(vnB_Detail, "standardPurchaseCost", {type:"NUMB", view:"", size:12, maxLen:12, desc:"成本", mode:"READONLY"});
	vat.item.make(vnB_Detail, "originalPrice", {type:"NUMB", view:"", size: 8, maxLen:12, desc:"定價", mode:"READONLY"});
	vat.item.make(vnB_Detail, "margenBeforeDisc", {type:"NUMB", view:"", size: 8, maxLen:12, desc:"折扣前毛利率", mode:"HIDDEN"}); //mode:"READONLY"(wade: 暫不顯示)  
	vat.item.make(vnB_Detail, "discountAmount", {type:"NUMB", view:"", size: 8, maxLen:12, desc:"促銷價", mask:"CCCCCCCCCCCC", onchange:"changeItemData()"});		
	vat.item.make(vnB_Detail, "discountPercentage", {type:"NUMB", view:"", size: 3, maxLen: 4, desc:"Discount", mask:"CCCCCCCCCCCC", onchange:"changeItemData()"});
	vat.item.make(vnB_Detail, "totalDiscountAmount", {type:"NUMB", view:"", size: 8, maxLen:12, desc:"折扣後金額", mode:"READONLY"});
	vat.item.make(vnB_Detail, "margenAfterDisc", {type:"NUMB", view:"", size: 8, maxLen:12, desc:"折扣後毛利率", mode:"HIDDEN"});//mode:"READONLY"(wade: 暫不顯示) 	
	vat.item.make(vnB_Detail, "totalDiscountPercentage", {type:"NUMB", view:"shift", size: 3, maxLen: 4, desc:"折扣", mode:"READONLY"});
	vat.item.make(vnB_Detail, "quantity", {type:"NUMB", view:"shift", size: 8, maxLen: 8, desc:"預計銷量"});
	vat.item.make(vnB_Detail, "stockOnHandQty", {type:"NUMB", view:"shift", size: 8, maxLen: 8, desc:"可用庫存量", mode:"HIDDEN"});//mode:"READONLY"(wade: 暫不顯示)
	vat.item.make(vnB_Detail, "reserve5", {type:"TEXT", view:"shift", size: 12, maxLen:20, desc:"備註"});	
...
function allShopInitial(){
	...
	title:"", rows:[
	 ...
	 {items:[{name:"#F.beginDate", type:"DATE",  bind:"beginDate", size:1}]}, 		 
	 {items:[{name:"#L.endDate", type:"LABEL" , value:"&nbsp;&nbsp; ~ &nbsp;&nbsp;結束日期"}]},	 
	 {items:[{name:"#F.endDate", type:"DATE",  bind:"endDate", size:1}]}]}], 
...
function shopInitial(){
	...
	vat.item.make(vnB_Shop_Detail, "beginDate", {type:"DATE", size: 8, maxLen:12, desc:"活動啟始日"});
	vat.item.make(vnB_Shop_Detail, "endDate", {type:"DATE", size: 8, maxLen:12, desc:"活動結束日"});
...
function loadBeforeAjxService(){
	var processString = "process_object_name=imPromotionMainService&process_object_method_name=getAJAXPageData" + 
	...
	if(vDiscountAmountTmp != null && vDiscountAmountTmp != "" && vDiscountPercentageTmp != null && vDiscountPercentageTmp != ""){
	    ...
	    vat.ajax.XHRequest(
        {
            ...
            find: function changeItemDataRequestSuccess(oXHR){     
                ...
                vat.item.setGridValueByName("stockOnHandQty", nItemLine, vat.ajax.getValue("StockOnHandQty", oXHR.responseText));
                vat.item.setGridValueByName("margenBeforeDisc", nItemLine, vat.ajax.getValue("MrgenBeforeDisc", oXHR.responseText));
                vat.item.setGridValueByName("margenAfterDisc", nItemLine, vat.ajax.getValue("MargenAfterDisc", oXHR.responseText));
            }
        });
    }
}
...
function allShopCheckedMsg(){
    if(document.forms[0]["#F.isAllShop"].checked){
        alert("點選所有專櫃後，此促銷方案將適用所有專櫃，下方之專櫃設定將不適用!!");
    }
}

function allCustomerCheckedMsg(){
    if(document.forms[0]["#F.isAllCustomer"].checked){
        alert("點選所有客戶後，此促銷方案將適用所有客戶，下方之類別設定將不適用!!");
    }
}

function getTotalAmt(){
    var processString = "process_object_name=imPromotionMainService&process_object_method_name=performCountTotalAmount" + 
	                "&headId=" + vat.item.getValueByName("#F.headId");
    vat.ajax.startRequest(processString, function () {
	    if (vat.ajax.handleState()) {
		    vat.item.setValueByName("#F.totalCostAmount", vat.ajax.getValue("TotalCostAmount", vat.ajax.xmlHttp.responseText));
            vat.item.setValueByName("#F.totalPriceAmount", vat.ajax.getValue("TotalPriceAmount", vat.ajax.xmlHttp.responseText)); 
            vat.item.setValueByName("#F.totalDiscountAmount",  vat.ajax.getValue("TotalDiscountAmount", vat.ajax.xmlHttp.responseText)); 
            vat.item.setValueByName("#F.totalAfterDiscountAmount", vat.ajax.getValue("TotalAfterDiscountAmount", vat.ajax.xmlHttp.responseText)); 
            vat.item.setValueByName("#F.averageDiscountRate", vat.ajax.getValue("AverageDiscountRate", vat.ajax.xmlHttp.responseText));  			
		}		
	});
}
...
function doFormAccessControl(){
	...
	vat.item.setAttributeByName("#F.promotionName", "readOnly", false);
	vat.item.setAttributeByName("#F.promotionNo", "readOnly", false);
	vat.item.setAttributeByName("#F.inCharge", "readOnly", false);
	vat.item.setAttributeByName("#F.description", "readOnly", false);
	vat.item.setAttributeByName("#F.itemCategory", "readOnly", false);
	vat.item.setAttributeByName("#F.purchaseAssist", "readOnly", false);
	vat.item.setAttributeByName("#F.purchaseMember", "readOnly", false);
	vat.item.setAttributeByName("#F.purchaseMaster", "readOnly", false);	
	//vat.item.setAttributeByName("#F.isAllShop", "readOnly", false);
	//vat.item.setAttributeByName("#F.beginDate", "readOnly", false);
	//vat.item.setAttributeByName("#F.endDate", "readOnly", false);
	//vat.item.setAttributeByName("#F.isAllCustomer", "readOnly", false);
	//vat.item.setAttributeByName("#F.customerType", "readOnly", false);
	...
	if("T2" == vsBrandCode){
	    ...
	    vat.item.setAttributeByName("#F.purchaseAssist", "readOnly", true);
	    vat.item.setAttributeByName("#F.purchaseMember", "readOnly", true);
	    vat.item.setAttributeByName("#F.purchaseMaster", "readOnly", true);	
	}	
    vat.item.setAttributeByName("vatItemDiv"     , "readOnly", false);
	vat.item.setAttributeByName("vatShopDiv"     , "readOnly", false);
	vat.item.setAttributeByName("vatCustomerDiv" , "readOnly", false);
	
	if( "T2" != vsBrandCode ){
		vat.block.column("category02", {view:"none"});
		vat.block.column("category02Name", {view:"none"});
	}
		
	vat.block.canGridModify([vnB_Detail, vnB_Shop_Detail, vnB_Customer_Detail], true, true, true);
	...
	if(  vsFormStatus != ""   ){
		...
		if(vsProcessId == 0 || vsProcessId == null){//使用Picker或從查詢功能選入  
			if("TMP" == vsOrderNoPrefix){
				...
				vat.item.setGridAttributeByName("category02", "readOnly", true);
