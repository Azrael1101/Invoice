...
function kweImHeader(){ 
	...
	title:"海關進出倉訊算", rows:[  
	 		{row_style:"", cols:[
	 		     {items:[{name:"#L.declNo", 		type:"LABEL", 	value:"單號"}]},
				 {items:[{name:"#F.startOrderNo", 		type:"TEXT", 	bind:"startOrderNo", size:25 ,value1:"AA  9463058016"},
				         {name:"#L.declNoTo", 		type:"LABEL", 	value:" 至 "},
	 		             {name:"#F.endOrderNo", 		type:"TEXT", 	bind:"endOrderNo", size:25 ,value1:"AA  9463058016" }]}
			]}
		], 	 
		beginService:"",
		closeService:""			
...
function kweImDetail(){
  ...
  var vbCanGridModify = true;
	...
	vat.item.make(vnB_Detail, "bondNo"           , {type:"TEXT" , size:15, maxLen:20, mode:"READONLY", desc:"海關監管編號"      });
	vat.item.make(vnB_Detail, "declNo"           , {type:"TEXT" , size:15, maxLen:20, mode:"READONLY", desc:"報單號碼(注意空白)"      });
	vat.item.make(vnB_Detail, "importDate"       , {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"進口日期"      });
	vat.item.make(vnB_Detail, "rlsTime"          , {type:"TEXT" , size:18, maxLen:20, mode:"READONLY", desc:"放行時間"      });
	vat.item.make(vnB_Detail, "stgPlace"        , {type:"TEXT"  , size:18, maxLen:20, mode:"READONLY", desc:"存放處所"      });
...
function loadBeforeAjxService(){
	var processString = "process_object_name=cmDeclarationHeadService&process_object_method_name=getAJAXSearchPageData" + 
...
function saveBeforeAjxService() {
	...
	if (checkEnableLine()) {
		processString = "process_object_name=cmDeclarationHeadService&process_object_method_name=saveSearchResult";
//		alert(processString); 
	}
	return processString;
}								
	...
	//vat.bean().vatBeanPicker.xxx = 1;
	   ...
	   vat.block.pageSearch(vnB_Detail, {
    		funcSuccess : 
    		    function(){
    		      vat.block.submit(
    		                     function(){ return "process_object_name=cmDeclarationHeadService&process_object_method_name=getSearchSelection";
